...
import { ConfigDrivenBalancer } from './ConfigDrivenBalancer';
...
function buildStatBlockFromSimValues(simValues: Record<string, number>): StatBlock {
  ...
  const numericBase = base as unknown as Record<string, number>;
  Object.entries(simValues).forEach(([id, value]) => {
    if (!Object.prototype.hasOwnProperty.call(base, id)) {
      return;
    }
    const key = id as keyof StatBlock;
    const current = base[key];
    if (typeof current === 'number' && typeof value === 'number') {
      numericBase[key] = value;
    }
  });
  return base;
}
...
export const Balancer: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<'legacy' | 'config-driven'>('legacy');
        ...
        <div className="observatory-shell space-y-3">
          <header className="flex items-center justify-between gap-2 flex-wrap">
            ...
            <div className="flex items-center gap-1.5 md:gap-2 mt-2 md:mt-0">
              {/* View Mode Toggle */}
              <div className="flex items-center gap-1 px-2 py-1 rounded-full border border-slate-600 bg-slate-800/50">
                <button
                  type="button"
                  onClick={() => setViewMode('legacy')}
                  className={`px-2 py-1 rounded-full text-[9px] tracking-[0.2em] uppercase transition-colors ${
                    viewMode === 'legacy'
                ...
                <button
                  type="button"
                  onClick={() => setViewMode('config-driven')}
                  className={`px-2 py-1 rounded-full text-[9px] tracking-[0.2em] uppercase transition-colors ${
                    viewMode === 'config-driven'
                      ? 'bg-indigo-500/20 text-indigo-200 border border-indigo-400/40'
                      : 'text-slate-400 hover:text-slate-200'
                  }`}
                >
              ...
              <button
                type="button"
                onClick={() => setShowStressTesting((prev) => !prev)}
          ...
          {/* Phase 10.5 Stress Testing Integration */}
          {showStressTesting && (
