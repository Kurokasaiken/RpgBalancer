...
export const ConfigToolbar: React.FC = () => {
  ...
  const [isHistoryBusy, setIsHistoryBusy] = React.useState(false);
  const [lastOperation, setLastOperation] = React.useState<string>('');

  const handleUndo = async () => {
    if (!canUndo || isHistoryBusy) return;
    ...
    try {
      await undo();
      setLastOperation('Undo successful');
      showToast('Operazione annullata', 'info');
    } catch (error) {
      setLastOperation('Undo failed');
      showToast(`Errore durante annullamento: ${(error as Error).message}`, 'error');
    } finally {
  ...
  const handleRedo = async () => {
    if (!canRedo || isHistoryBusy) return;
    ...
    try {
      await redo();
      setLastOperation('Redo successful');
      showToast('Operazione ripristinata', 'info');
    } catch (error) {
      setLastOperation('Redo failed');
      showToast(`Errore durante ripristino: ${(error as Error).message}`, 'error');
    } finally {
      setIsHistoryBusy(false);
    }
  };
      ...
      <div className="mb-4 rounded-2xl border border-[#384444] bg-linear-to-r from-[#0b181b]/85 via-[#081013]/90 to-[#0b181b]/85 px-3 py-2 shadow-[0_10px_30px_rgba(0,0,0,0.6)] text-[10px] flex flex-wrap items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-1.5">
          <button
            ...
            className={`inline-flex items-center gap-1 px-2 py-1 rounded-full border transition-all duration-200 ${canUndo && !isHistoryBusy
                ? 'border-amber-400/70 text-amber-200 hover:bg-amber-500/15 hover:border-amber-400'
                : 'border-[#2b3434] text-[#4b5555] cursor-not-allowed'
              }`}
            title={canUndo ? 'Annulla ultima modifica' : 'Nessuna operazione da annullare'}
          >
          ...
          <button
            ...
            className={`inline-flex items-center gap-1 px-2 py-1 rounded-full border transition-all duration-200 ${canRedo && !isHistoryBusy
                ? 'border-emerald-400/70 text-emerald-200 hover:bg-emerald-500/15 hover:border-emerald-400'
                : 'border-[#2b3434] text-[#4b5555] cursor-not-allowed'
              }`}
            title={canRedo ? 'Ripristina operazione' : 'Nessuna operazione da ripristinare'}
          >
            <span aria-hidden className="text-xs">↻</span>
            <span className="tracking-[0.18em] uppercase">
              {isHistoryBusy ? '...' : 'Redo'}
