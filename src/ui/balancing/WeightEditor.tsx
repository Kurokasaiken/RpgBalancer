...
interface WeightEditorProps {
    ...
    onSave: (newPresetId: string) => void | Promise<void>;
    onClose: () => void;
}
...
export const WeightEditor: React.FC<WeightEditorProps> = ({
    ...
    const handleSaveAsNew = async () => {
        ...
        try {
            const newPreset = await createUserPreset(
                presetName,
            ...
            try {
                await onSave(newPreset.id);
                toast.success('Preset created!', {
                    description: `"${newPreset.name}" saved and activated`
                });
                onClose();
            } catch (activationError) {
                toast.error('Preset saved but activation failed', {
                    description: (activationError as Error).message
                });
            }
        } catch (error) {
        ...
        input.onchange = (e) => {
            ...
            reader.onload = async (event) => {
                try {
                    ...
                    const imported = await importPresetJSON(json);

                    try {
                        await onSave(imported.id);
                        toast.success('Preset imported!', {
                            description: `"${imported.name}" added and activated`,
                        });
                        onClose();
                    } catch (activationError) {
                        toast.error('Preset imported but activation failed', {
                            description: (activationError as Error).message
                        });
                    }
                } catch (error) {
