...
export const StatEditor: React.FC<Props> = ({ isOpen, onClose, card, editingStat, onSaveComplete }) => {
  ...
  const [id, setId] = useState(editingStat?.id || '');
  const [label, setLabel] = useState(editingStat?.label || '');
  const [description, setDescription] = useState(editingStat?.description || '');
  const [type, setType] = useState<'number' | 'percentage'>(editingStat?.type || 'number');
  const [min, setMin] = useState(editingStat?.min ?? 0);
  const [max, setMax] = useState(editingStat?.max ?? 100);
  const [step, setStep] = useState(editingStat?.step ?? 1);
  const [defaultValue, setDefaultValue] = useState(editingStat?.defaultValue ?? 0);
  const [weight, setWeight] = useState(editingStat?.weight ?? 1);
  const [isDerived, setIsDerived] = useState(editingStat?.isDerived ?? false);
  const [formula, setFormula] = useState(editingStat?.formula || '');
  const [isPenalty, setIsPenalty] = useState(!!editingStat?.isPenalty);
   
  const [_baseStat, _setBaseStat] = useState(editingStat?.baseStat ?? (!editingStat?.isDerived && !editingStat?.isPenalty));
   
  const [_isDetrimental, _setIsDetrimental] = useState(editingStat?.isDetrimental ?? !!editingStat?.isPenalty);
  const [error, setError] = useState<string | undefined>();

  // Remove the useEffect that sets state synchronously
  // Instead, rely on key prop to remount component when editingStat changes
  ...
  return (
    <div key={`${editingStat?.id || 'new'}-${isOpen ? 'open' : 'closed'}`} className="fixed inset-0 z-40 flex justify-end bg-black/60">
