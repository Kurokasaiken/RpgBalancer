...
describe('Balancer - Import/Export', () => {
  ...
  it('should export all core stats', async () => {
    const exported = await BalancerConfigStore.export();
  ...
  it('should preserve all cards in export', async () => {
    const exported = await BalancerConfigStore.export();
    const parsed = JSON.parse(exported) as BalancerConfig;
  ...
  it('should import valid JSON config', async () => {
    const exported = await BalancerConfigStore.export();
    const imported = await BalancerConfigStore.import(exported);
  ...
  it('should preserve formulas after import', async () => {
    const exported = await BalancerConfigStore.export();
    const imported = await BalancerConfigStore.import(exported);
  ...
  it('should round-trip export/import correctly', async () => {
    const original = await BalancerConfigStore.load();
    const exported = await BalancerConfigStore.export();
    const imported = await BalancerConfigStore.import(exported);
  ...
  it('should handle invalid JSON import gracefully', async () => {
    ...
    await expect(async () => {
      await BalancerConfigStore.import(invalidJson);
    }).rejects.toThrow();
  });

  it('should preserve stat properties after import', async () => {
    const exported = await BalancerConfigStore.export();
    const imported = await BalancerConfigStore.import(exported);
  ...
  it('should preserve card properties after import', async () => {
    const exported = await BalancerConfigStore.export();
    const imported = await BalancerConfigStore.import(exported);
  ...
  it('should preserve preset data after import', async () => {
    const exported = await BalancerConfigStore.export();
    const imported = await BalancerConfigStore.import(exported);
  ...
  it('should maintain config integrity after multiple export/import cycles', async () => {
    let config = await BalancerConfigStore.load();
    
    for (let i = 0; i < 3; i++) {
      const exported = await BalancerConfigStore.export();
      config = await BalancerConfigStore.import(exported);
    }
  ...
  it('should export with proper formatting', async () => {
    const exported = await BalancerConfigStore.export();
  ...
  it('should handle empty/null values in import', async () => {
    const config = await BalancerConfigStore.load();
    const exported = JSON.stringify(config, null, 2);
    
    await expect(BalancerConfigStore.import(exported)).resolves.not.toThrow;
  });
});
