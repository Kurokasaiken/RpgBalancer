...
import { CharacterBuilder, type CharacterBuilderSavePayload } from './CharacterBuilder';
import {
    saveCharacter,
    loadCharacters,
    deleteCharacter,
...
import { DEFAULT_PORTRAIT_CROP, matchVisualProfile } from '@/balancing/config/idleVillage/residentVisuals';
import { PortraitPreviewModal } from '@/ui/character/components/PortraitPreviewModal';
import {
    PORTRAIT_BADGE_CONTAINER_CLASS,
    PORTRAIT_IMAGE_CLASS,
    getPortraitImageStyle,
} from '@/ui/character/components/portraitStyles';
import { useIdleVillageConfig } from '@/balancing/hooks/useIdleVillageConfig';
import { regenerateResidentsFromRoster } from '@/engine/game/idleVillage/regenerateResidentsFromRoster';
...
export const CharacterManager: React.FC = () => {
    ...
    const resolveCharacterPortrait = useCallback((character: SavedCharacter) => {
        const profile = matchVisualProfile({
            profileId: character.visualProfileId,
            statProfileId: character.aiBehavior,
            residentId: character.id,
        });
        const portraitUrl =
            character.portraitUrl && character.portraitUrl.trim().length > 0
                ? character.portraitUrl
                : profile?.portrait?.src;
        const fullFigureUrl =
            character.fullFigureUrl && character.fullFigureUrl.trim().length > 0
                ? character.fullFigureUrl
                : profile?.fullFigure?.src;
        return {
            portraitUrl: portraitUrl ?? undefined,
            fullFigureUrl: fullFigureUrl ?? undefined,
            profileLabel: profile?.label ?? 'Portrait',
            crop: character.portraitCrop ?? profile?.portrait?.defaultCrop ?? DEFAULT_PORTRAIT_CROP,
        };
    }, []);

    const openPortraitPreview = useCallback((image?: string | null, label?: string | null) => {
        if (!image) return;
        setPreviewImage(image);
        setPreviewLabel(label ?? 'Portrait');
    }, []);

    const closePortraitPreview = useCallback(() => {
        setPreviewImage(null);
        setPreviewLabel(null);
    }, []);

    const handleSaveCharacter = ({ combatant, visualProfileId, portraitUrl, fullFigureUrl, portraitCrop }: CharacterBuilderSavePayload) => {
        const savedChar: SavedCharacter = {
            ...
            fullFigureUrl,
            portraitCrop,
            // ResidentState properties defaults
            status: 'available',
            fatigue: 0,
            currentHp: combatant.entity.statBlock!.hp,
            maxHp: combatant.entity.statBlock!.hp,
            isInjured: false,
            isHero: false,
            survivalCount: 0,
            survivalScore: 0,
    ...
    const handleRegenerateResidents = useCallback(() => {
        if (!config) {
            setRegenerationStatus(null);
            setRegenerationError('Config Idle Village non disponibile.');
            return;
        }
        ...
        try {
            const result = regenerateResidentsFromRoster(config, {
                description: 'Rigenera residenti da Character Manager',
            });
            if (result.residentsCreated === 0) {
                setRegenerationStatus('Nessun personaggio salvato da rigenerare.');
            } else {
                setRegenerationStatus(`Rigenerati ${result.residentsCreated} PG in Idle Village.`);
            }
        } catch (error) {
            console.error('Impossibile rigenerare i residenti Idle Village:', error);
            setRegenerationError('Errore durante la rigenerazione dei PG.');
        }
    }, [config]);
            ...
            <div className="flex flex-col h-full space-y-4 relative z-10">
                <div className="backdrop-blur-md bg-white/5 border border-white/10 p-6 rounded-xl shadow-[0_4px_16px_rgba(0,0,0,0.3)]">
                    ...
                    <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:items-center">
                        <button
                            type="button"
                            onClick={handleRegenerateResidents}
                            className="inline-flex items-center justify-center rounded-lg border border-amber-400/40 bg-amber-500/10 px-4 py-2 text-sm font-semibold uppercase tracking-[0.25em] text-amber-200 shadow-[0_0_15px_rgba(251,191,36,0.25)] transition hover:bg-amber-500/20 hover:border-amber-300/70"
                        >
                            🔁 Rigenera Idle Village
                        </button>
                        <div className="text-xs">
                            {regenerationStatus && <p className="text-emerald-300">{regenerationStatus}</p>}
                            {regenerationError && <p className="text-red-300">{regenerationError}</p>}
                        </div>
                    </div>
                </div>
                                        ...
                                        >
                                            <div className="flex justify-between items-start mb-2">
                                                <div className="flex items-center gap-3">
                                                    <button
                                                        type="button"
                                                        className={`${PORTRAIT_BADGE_CONTAINER_CLASS} h-12 w-12`}
                                                        onClick={() => openPortraitPreview(resolvedPortrait, profileLabel)}
                                                        disabled={!resolvedPortrait}
                                                    >
                                                        {resolvedPortrait ? (
                                                            <img
                                                                ...
                                                                style={getPortraitImageStyle(crop)}
                                                            />
                                                        ) : (
                                                ...
                                                <div className="flex space-x-2">
                                                    {fullFigureUrl && (
                                                        <button
                                                            onClick={() => openPortraitPreview(fullFigureUrl, `${char.name} – Full Figure`)}
                                                            className="px-3 py-1 bg-white/5 border border-white/10 text-white rounded text-xs uppercase tracking-[0.35em] hover:border-emerald-300/60 transition"
                                                        >
                                                            View
                                                        </button>
                                                    )}
