...
export const CharacterSelector: React.FC<CharacterSelectorProps> = ({ onCharactersSelected }) => {
    const characters = useState<SavedCharacter[]>(() => loadCharacters())[0];
    const spells = useState<Spell[]>(() => loadSpells())[0];
    ...
    useEffect(() => {
        ...
        if (characters.length >= 2) {
            ...
            setSelectedChar1(characters[0]);
             
            setSelectedChar2(characters[1]);
        }
    }, [characters]);
    ...
    return (
        <div className="w-full h-full flex flex-col items-center justify-center p-4 bg-linear-to-b from-gray-900 to-gray-800 overflow-y-auto">
            <div className="max-w-6xl w-full bg-gray-800/90 rounded-xl border-2 border-gray-700 p-4 md:p-6 shadow-2xl">
              ...
              ${selectedChar1 && selectedChar2 && selectedChar1.id !== selectedChar2.id
                                ? 'bg-linear-to-r from-blue-600 to-purple-600 text-white hover:from-blue-700 hover:to-purple-700 shadow-lg hover:shadow-xl transform hover:scale-105'
