...
export interface CertifiedWorkerPickerSheetProps {
  isOpen: boolean;
...
const CertifiedWorkerPickerSheet: React.FC<CertifiedWorkerPickerSheetProps> = ({
  isOpen,
  slotMeta,
  residents,
  onAssign,
  ...
  const getFocusableElements = useCallback(() => {
    ...
    return Array.from(sheetRef.current.querySelectorAll<HTMLElement>(focusableSelectors)).filter(
  ...
  const handleEscapeKey = useCallback(
    ...
    [emitTelemetry, isOpen, onClose, slotMeta?.slotId],
  );
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      ...
      emitTelemetry({ type: 'open', slotId: slotMeta?.slotId ?? null, candidateCount: residents.length });
      emitTelemetry({ type: 'candidate_count', slotId: slotMeta?.slotId ?? null, candidateCount: residents.length });
    ...
    return undefined;
  }, [emitTelemetry, focusPrimaryAction, handleEscapeKey, isOpen, residents.length, slotMeta?.slotId]);
  ...
  const handleAssignResident = useCallback(
    async (residentId: string) => {
      ...
      const latencyMs = assignStartRef.current !== null ? performance.now() - assignStartRef.current : undefined;
    ...
    [emitTelemetry, onAssign, residents, scheduleCloseTimeout, slotMeta?.slotId],
  );
  ...
  const headerCopy = trigger === 'keyboard' ? 'Selezione tastiera' : slotMeta?.description ?? 'Selezione slot';
  ...
  const sheetStyle = useMemo(
    () => ({
      ...
      boxShadow: '0 -35px 80px rgba(0, 0, 0, 0.65), inset 0 1px 0 rgba(255, 255, 255, 0.08)',
  ...
  const headerStyle = useMemo(
    () => ({
      background: 'linear-gradient(135deg, rgba(8, 12, 20, 0.95), rgba(3, 5, 10, 0.9))',
  ...
  const sheetContent = (
    ...
    >
      <section
        ...
        style={sheetStyle}
      >
        <span tabIndex={0} aria-hidden="true" className="absolute h-0 w-0 opacity-0" onFocus={focusLastElement} />
        <div className="sticky top-0 z-10 border-b px-5 py-4 backdrop-blur" style={headerStyle}>
          <div className="flex items-start justify-between gap-3">
            <div className="space-y-1">
              <p id={descriptionId} className="text-[11px] uppercase tracking-[0.35em] text-slate-400">
                {headerCopy}
              </p>
              <h2 id={titleId} className="text-lg font-semibold text-(--text-primary,#fefce8)">
                {slotMeta?.label ?? 'Seleziona slot'}
              </h2>
              {slotMeta?.activityLabel && <p className="text-xs text-slate-400">{slotMeta.activityLabel}</p>}
            </div>
            <div className="flex items-center gap-2">
              <div className="hidden h-6 w-6 rounded-full border border-white/10 sm:block" aria-hidden="true" id={decorativeOrbId} />
              <button
                type="button"
        ...
        <div className="max-h-[60vh] space-y-3 overflow-y-auto px-5 py-4">
          {residents.length === 0 ? (
            <p className="text-sm text-slate-400">Nessun residente compatibile per questo slot in questo momento.</p>
          ) : (
            residents.map((resident, index) => {
              ...
              return (
                <article
                  ...
                  data-testid={`worker-picker-row-${resident.id}`}
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="mt-1 inline-flex items-center gap-2 text-[11px] text-slate-500">
                        <span className="rounded-full border border-emerald-400/40 px-2 py-0.5 text-emerald-200">{scorePercent}% compatibilità</span>
        ...
        <span tabIndex={0} aria-hidden="true" className="absolute h-0 w-0 opacity-0" onFocus={focusFirstElement} />
      </section>
    </div>
  );
...
export default CertifiedWorkerPickerSheet;
