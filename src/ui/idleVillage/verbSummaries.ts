...
import type { ProgressStyle, VerbTone, VerbVisualVariant } from '@/ui/idleVillage/legacy/VerbCard';
import {
...
const buildRiskStripeMetrics = (injuryPercentage: number, deathPercentage: number): TheaterRiskStripeMetrics =>
  deriveTheaterRiskStripes({ injuryPercentage, deathPercentage });
...
export interface VerbSummary {
  ...
  /** Optional quest phase sequence for multi-phase quests */
  currentPhaseIndex?: number;
  /** Optional quest phase sequence for multi-phase quests */
  totalPhases?: number;
  /** Optional quest phase sequence for multi-phase quests */
  currentPhaseType?: string;
  /** Optional precomputed risk stripe metrics shared with ActivityActionCard */
...
export function buildActivityBlueprintSummary(params: {
  ...
  const riskStripeMetrics = buildRiskStripeMetrics(injury, deathPercentage);
...
export function buildPassiveEffectSummary(params: {
  ...
  const riskStripeMetrics = buildRiskStripeMetrics(0, 0);
...
export const deriveIcon = (activity: ActivityDefinition, slotIcon?: string): ReactNode => {
...
export function createRiskLabel(injuryPct: number, deathPct: number): string | null {
  const parts: string[] = [];
...
export function buildQuestOfferSummary(params: {
  ...
  const riskStripeMetrics = buildRiskStripeMetrics(injury, death);
