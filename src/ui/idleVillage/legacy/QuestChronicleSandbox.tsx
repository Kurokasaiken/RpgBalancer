...
import type { QuestState, QuestPhaseResult, QuestPhaseType } from '@/balancing/config/idleVillage/types';

const VISUAL_VARIANT_BY_PHASE: Record<QuestPhaseType, 'amethyst' | 'ember' | 'jade'> = {
  check: 'amethyst',
  fight: 'ember',
  explore: 'amethyst',
  stealth: 'amethyst',
...
const QuestChronicleSandbox = () => {
  ...
  const verbCards = useMemo(() => {
    ...
    return questBlueprint.phases.map((phase, index) => {
      ...
      const injury = phase.type === 'fight' ? 35 : phase.type === 'check' ? 18 : 6;
      const death = phase.type === 'fight' ? 12 : 0;
