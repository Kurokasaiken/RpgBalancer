  ...
  const slotViewModels = useMemo<ResidentSlotViewModel[]>(
    () =>
      slots.map((slot, index) => {
        const dropState = slotDropStates?.[slot.id] ?? 'idle';
        const bloomState = dropState === 'valid' ? 'valid' : dropState === 'invalid' ? 'invalid' : 'idle';
        return {
          id: slot.id,
          index,
          label: slot.label,
          ...
          dropState,
          bloomState,
          status: slot.assignedResidentId ? 'assigned' : 'empty',
          telemetryTags: slot.telemetryTags ?? [],
        };
      }),
        ...
        {slotViewModels.length > 0 && (
          <div className="space-y-3">
            ...
            <ResidentSlotRack
              ...
              layout="detail"
              overflowBehavior={slotViewModels.length > 4 ? 'scroll' : 'wrap'}
