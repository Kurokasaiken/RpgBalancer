            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <label className="block font-bold mb-1">
                ...
                <input
                  type="number"
                  min={1}
                  value={rules.ticksPerDay ?? ''}
                  onChange={(e) => handleNumberChange('ticksPerDay')(e.target.value)}
                  className="mt-1 w-full px-2 py-1 bg-obsidian border border-slate rounded text-ivory"
                  placeholder={`${rules.dayLengthInTimeUnits} (default)`}
                />
                <span className="block text-[11px] text-slate-400">Override tick count used per daylight cycle</span>
              </label>
              <label className="block font-bold mb-1">
                Ticks per Night
                <input
                  type="number"
                  min={1}
                  value={rules.ticksPerNight ?? ''}
                  onChange={(e) => handleNumberChange('ticksPerNight')(e.target.value)}
                  className="mt-1 w-full px-2 py-1 bg-obsidian border border-slate rounded text-ivory"
                  placeholder={`${dayNight.nightTimeUnits} (default)`}
                />
                <span className="block text-[11px] text-slate-400">Override tick count used per night cycle</span>
              </label>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <label className="block font-bold mb-1">
                Fatigue Recovery per Night Tick
                <input
                  type="number"
                  min={0}
                  step={0.1}
                  value={rules.fatigueRecoveryPerNightTick ?? ''}
                  onChange={(e) => handleNumberChange('fatigueRecoveryPerNightTick')(e.target.value)}
                  className="mt-1 w-full px-2 py-1 bg-obsidian border border-slate rounded text-ivory"
                  placeholder={`${(rules.fatigueRecoveryPerDay / (rules.ticksPerNight ?? dayNight.nightTimeUnits)).toFixed(2)}`}
                />
                <span className="block text-[11px] text-slate-400">
                  Lascia vuoto per derivare da Fatigue Recovery / ticks per night
                </span>
              </label>
              <label className="block font-bold mb-1">
                Production Halt Threshold (0-1)
                <input
                  type="number"
                  min={0}
                  max={1}
                  step={0.05}
                  value={rules.productionHaltFatigueThreshold ?? 1}
                  onChange={(e) => handleNumberChange('productionHaltFatigueThreshold')(e.target.value)}
                  className="mt-1 w-full px-2 py-1 bg-obsidian border border-slate rounded text-ivory"
                />
                <span className="block text-[11px] text-slate-400">
                  Fatigue ratio beyond which jobs smettono di produrre
                </span>
              </label>
            </div>
            <div>
