/**
 * PoiMarkerRunicV1 — fantasy variant of the opportunity marker.
 *
...
export const PoiMarkerRunicV1: React.FC<PoiMarkerProps> = ({
  type,
  state = 'available',
  progress = 1,
  durationMs,
  ...
  const reactId = useId();
  const id = `poiv1-${reactId.replace(/[^a-zA-Z0-9_-]/g, '')}`;
  ...
  const cssVars = {
    '--poiv1-core': ember.core,
    '--poiv1-light': ember.light,
    '--poiv1-deep': ember.deep,
    '--poiv1-size': `${size}px`,
  } as CSSProperties;
  ...
  return (
    <button
      ...
      className={[
        ...
        `poiv1--${type}`,
        `poiv1--${state}`,
        importance !== 'normal' ? `poiv1--${importance}` : '',
        disabled ? 'poiv1--disabled' : '',
        className,
      ]
        .filter(Boolean)
        .join(' ')}
      ...
      disabled={disabled}
    >
      <svg className="poiv1__svg" viewBox="0 0 120 120" role="img" aria-hidden="true">
        <defs>
        ...
        {sealProgress > 0.001 && (
          <g className="poiv1__seal" filter={`url(#${id}-magic-glow)`}>
            {outerBand.map((g, i) => (
              <g key={`s-${i}`}>
                <path
                  d={g.outerD}
        ...
        <g className="poiv1__rim">
          {rimBand.map((g, i) => (
            <g
              key={`r-${i}`}
              transform={`translate(${g.x.toFixed(2)} ${g.y.toFixed(2)}) rotate(${g.a})`}
              opacity={0.6 + 0.4 * g.lit}
              filter={g.lit > 0 ? `url(#${id}-magic-glow)` : undefined}
            >
        ...
        <circle
          ...
          fill={`url(#${id}-hearth)`}
          opacity={1}
          className="poiv1__hearth"
        />

        {/* Bas-relief glyph: sunken copy first, lit copy on top. */}
        <g transform="translate(60 60)" className="poiv1__emblem" opacity={1}>
...
export const poiRunicV1Styles = `
.poiv1 {
  --poiv1-size: 112px;
  position: relative;
  display: inline-flex;
  width: var(--poiv1-size);
  height: var(--poiv1-size);
...
.poiv1:focus-visible { outline: none; box-shadow: 0 0 0 2px var(--poiv1-light); border-radius: 50%; }
.poiv1:hover:not(:disabled) { transform: scale(1.035); }
.poiv1:active:not(:disabled) { transform: scale(.985); }

.poiv1__svg { width: 100%; height: 100%; overflow: visible; }

.poiv1__body { transform-origin: 60px 60px; animation: poiv1-breathe 8s ease-in-out infinite; }

/* The magic seal reveals progressively as time passes. */
.poiv1__seal { transform-origin: 60px 60px; }

.poiv1--new .poiv1__body {
  animation: poiv1-arrive 720ms cubic-bezier(.2,.9,.2,1) both,
             poiv1-breathe 8s ease-in-out 720ms infinite;
}
.poiv1--new .poiv1__seal {
  animation: poiv1-summon 900ms ease-out both,
             poiv1-drift 42s linear 900ms infinite;
}

.poiv1--disabled, .poiv1--expired { cursor: default; filter: saturate(.55) brightness(.8); }

@keyframes poiv1-breathe { 0%,100% { transform: scale(1); } 50% { transform: scale(1.008); } }

@media (prefers-reduced-motion: reduce) {
  .poiv1, .poiv1 * { animation: none !important; transition: none !important; }
}
`;

export default PoiMarkerRunicV1;
