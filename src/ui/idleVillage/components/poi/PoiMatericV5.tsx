...
import { POI_MATERIC_V5_TOKENS } from '@/balancing/config/idleVillage/poiMatericV5Tokens';
import type { PoiMarkerProps } from './PoiMarker';

/**
 * POI Materic V5 — bronze is the matter, light is the variable.
 *
 ...
 *
 * Every visual value comes from `poiMatericV5Tokens`; nothing is hardcoded.
 *
 ...
 */
export interface PoiMatericV5Props extends PoiMarkerProps {
  isDragging?: boolean;
  'data-testid'?: string;
}
...
export const PoiMatericV5: React.FC<PoiMatericV5Props> = ({
  type,
  state = 'available',
  progress = 1,
  ...
  const iconPass = (
    ...
    <g transform={`translate(${dx} ${dy})`} opacity={opacity} style={blendMode ? { mixBlendMode: blendMode } : undefined}>
      ...
      >
        <defs>
          ...
          */}
          ...
          { /*
            Specular reflection sliding over the mark. A narrow white band that
            stays independent of the base metal colour, giving the arms a glossy
            skin rather than a flat recolour.
          */}
          <linearGradient
            id={gid('iconSpecular')}
            x1="0%"
          ...
          >
            <stop offset="0%" stopColor={T.metal.specular} stopOpacity="0" />
            <stop offset="38%" stopColor={T.metal.specular} stopOpacity="0" />
            <stop offset="46%" stopColor={T.metal.specular} stopOpacity="0.9" />
            <stop offset="54%" stopColor={T.metal.specular} stopOpacity="0" />
            <stop offset="100%" stopColor={T.metal.specular} stopOpacity="0" />
          </linearGradient>

          {/* Glossy ambient highlight on the curved glass/core, like the mock's sheen. */}
          <linearGradient
            id={gid('gloss')}
            x1="0%"
            y1="0%"
            ...
            gradientUnits="objectBoundingBox"
          >
            <stop offset="0%" stopColor={T.metal.specular} stopOpacity="0.35" />
            <stop offset="45%" stopColor={T.metal.specular} stopOpacity="0.08" />
            <stop offset="75%" stopColor={T.metal.specular} stopOpacity="0" />
          </linearGradient>

          {/* Gem: radial with a shifted bright centre to read as a curved, translucent stone. */}
          <radialGradient id={gid('gem')} cx="32%" cy="28%" r="70%">
            <stop offset="0%" stopColor={T.metal.specular} />
            <stop offset="35%" stopColor={identity.glaze} />
            <stop offset="70%" stopColor={identity.rail} />
            <stop offset="100%" stopColor={alloyDark} />
          </radialGradient>

          { /*
            Directional chiaroscuro across the ring band. A flat, cast-metal strip
            lit from upper-left: the alloy's own mid tone forms most of the band,
            then sinks into the alloy's shadow at the lower-right. More metal,
            less darkness — but still not the bright V4.
          */}
          <linearGradient
            id={gid('rimChiaroscuro')}
            x1="0%"
            y1="0%"
            x2="100%"
            y2="100%"
          ...
          >
            <stop offset="0%" stopColor={alloyLight} />
            <stop offset="10%" stopColor={alloyMid} />
            <stop offset="75%" stopColor={alloyMid} />
            <stop offset="100%" stopColor={alloyDark} />
          </linearGradient>

          {/* Bevel lip for the outer and inner walls of the ring band. */}
          <linearGradient
            id={gid('bevelEdge')}
            x1="0%"
            y1="0%"
          ...
          >
            <stop offset="0%" stopColor={alloyLight} />
            <stop offset="50%" stopColor={alloyMid} />
            <stop offset="100%" stopColor={alloyDark} />
          </linearGradient>

          {/* Vignette to deepen the black core — dark at the edges, transparent in the middle. */}
          <radialGradient id={gid('vignette')} cx="38%" cy="32%" r="72%">
            <stop offset="0%" stopColor={T.rim.ink} stopOpacity="0" />
            <stop offset="70%" stopColor={T.rim.ink} stopOpacity="0.35" />
            <stop offset="100%" stopColor={T.rim.ink} stopOpacity="0.75" />
          </radialGradient>

          { /*
            Cast-metal micro-texture. Fractal noise, desaturated, blended in
            overlay at very low opacity so the metal stops reading as flat vector.
          */}
          <filter
            id={gid('noise')}
            x="-20%"
          ...
          { /*
            Drop shadow for the central mark. Gives the icon real thickness and
            weight, like the star in the reference, without hand-drawing a shadow.
          */}
          <filter
            id={gid('markShadow')}
            x="-50%"
          ...
          >
            <feDropShadow
              ...
              dy="1.2"
              stdDeviation="1.4"
              floodColor={T.shadow.color}
              floodOpacity={T.shadow.opacity}
            />
          </filter>
        ...
        */}
        ...
        {sealProgress > 0.001 && (
          <>
            ...
            */}
            ...
            {/* Halo pass */}
            <g className="poiv5__seal-halo" filter={`url(#${gid('bloom')})`}>
        ...
        {/* Micro-texture over the cast body. White source is turned into grey noise by the filter. */}
        <circle
          cx={g.center}
          cy={g.center}
          r={g.medalRadius}
          ...
          opacity={T.surface.noiseOpacity}
        ...
        */}
        ...
        {/* Vignette: the center gets heavier, the edges sink into the recess. */}
        <circle cx={g.center} cy={g.center} r={g.fieldRadius} fill={`url(#${gid('vignette')})`} />

        {/*
          Glass gloss: a blurred, off-centre ellipse of light sitting on the curved
          surface of the black core, not inside the icon gradient.
        */}
        <ellipse
          className="poiv5__gloss"
          cx={g.center - 7}
          cy={g.center - 9}
          rx="12"
          ry="8"
        ...
        {/*
          The ring band: a flat strip of cast metal with directional chiaroscuro.
          The mid tone catches the upper-left light; the rest stays in the alloy's
          own shadow. The icons are engraved into this strip.
        */}

        {/* Inner wall of the ring band — the raised lip between field and runes. */}
        <circle
          className="poiv5__rim-inner-bevel"
          cx={g.center}
          cy={g.center}
          r={g.innerRingRadius}
          fill="none"
          stroke={`url(#${gid('bevelEdge')})`}
          strokeWidth="1.6"
        />

        <circle
          className="poiv5__rim-band"
          cx={g.center}
          cy={g.center}
          r={g.rimLetterRadius}
          fill="none"
          stroke={`url(#${gid('rimChiaroscuro')})`}
          strokeWidth={T.rim.bandWidth}
        />

        {/* Micro-texture over the rune band. */}
        <circle
          className="poiv5__rim-noise"
          cx={g.center}
          cy={g.center}
          r={g.rimLetterRadius}
          fill="none"
        ...
        <g className="poiv5__rim">
          {rimBand.map((l, i) => {
            const [lx, ly] = T.rim.lightShift;
            const [sx, sy] = T.rim.shadowShift;
            return (
              <g key={`rim-${i}`} transform={`translate(${l.x.toFixed(2)} ${l.y.toFixed(2)}) rotate(${l.angle})`}>
                <g transform={`scale(${g.rimLetterScale})`}>
                  {/* Lit upper-left lip of the incision. Mid tone, not the bright crown. */}
                  <path
                    d={l.d}
                    fill="none"
                    stroke={alloyMid}
                  ...
                  />
                  {/* Shadowed lower-right wall of the incision. */}
                  <path
                    d={l.d}
                    fill="none"
                    stroke={T.surface.innerShadow}
        ...
        {/* ---------- THE MARK ---------- */}
        <g
          className="poiv5__icon"
          transform={`translate(${g.center} ${g.center}) scale(0.9)`}
          filter={`url(#${gid('markShadow')})`}
        >
          ...
          {/* Glossy specular layer sliding over the metal, independent of the base colour. */}
          {iconPass(`url(#${gid('iconSpecular')})`, 0, 0, 0.85, 'screen')}
          {/* Micro highlight on the boss: off-centre, so it reads as spherical, not flat. */}
          <ellipse
            cx={-icon.boss * 0.25}
            cy={-icon.boss * 0.25}
            rx={icon.boss * 0.22}
            ry={icon.boss * 0.15}
            fill={T.metal.specular}
            opacity="0.85"
          />
        ...
        {/* Gem at the bottom of the ring: translucent stone with its own highlight. */}
        <g transform={`translate(${g.center} ${g.center + g.medalRadius + 4})`}>
          <circle r="3.6" fill={`url(#${gid('gem')})`} filter={`url(#${gid('markShadow')})`} />
          <ellipse
            cx="-1.1"
            cy="-1.1"
            rx="1.4"
 ...
 */
export const poiMatericV5Styles = `
.poiv5 { display: block; }
.poiv5__svg { position: relative; width: 100%; height: 100%; overflow: visible; }
.poiv5__body { transition: filter 250ms ease; }
.poiv5:hover .poiv5__body { filter: brightness(1.12); }
.poiv5__icon { transition: filter 250ms ease; }
.poiv5:hover .poiv5__icon { filter: contrast(1.12) brightness(1.12); }
@media (prefers-reduced-motion: reduce) {
  .poiv5__body, .poiv5__icon { transition: none; }
}
`;

export default PoiMatericV5;
