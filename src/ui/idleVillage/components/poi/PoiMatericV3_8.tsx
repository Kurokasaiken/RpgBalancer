...
import { POI_MATERIC_V3_8_TOKENS } from '@/balancing/config/idleVillage/poiMatericV3_8Tokens';
import type { PoiMarkerProps } from './PoiMarker';

export interface PoiMatericV3_8Props extends PoiMarkerProps {
  portraitUrl?: string;
  isDragging?: boolean;
...
const T8 = POI_MATERIC_V3_8_TOKENS;
...
export const PoiMatericV3_8: React.FC<PoiMatericV3_8Props> = ({
  type,
  state = 'available',
  importance = 'normal',
  progress = 1,
  ...
  const isCompact = sizePx < T8.compactBelowPx;
  const showAnchor = grounded !== false;
  ...
  return (
    <div
      ...
      className={`poiv3_8 tok-svg ${className}${burst > 0.001 ? ' poiv3_8--burst' : ''}`}
      onClick={onClick}
    ...
    >
      <div className="poiv3_8__vignette" aria-hidden="true" />
      <div className="poiv3_8__fog" aria-hidden="true" />
      <canvas
        ref={canvasRef}
        width={sizePx}
        height={sizePx}
      ...
      >
        ...
        >
          <defs>
            ...
            {/* Field: type-aware deep green shadow */}
            <radialGradient id={gid('field')} cx="38%" cy="30%" r="78%">
              <stop offset="0%" stopColor={fieldBackgroundGradient[0]} />
              <stop offset="100%" stopColor={fieldBackgroundGradient[1]} />
            </radialGradient>
          ...
          {sealProgress > 0.001 && (
            <>
              ...
              <g className="poiv3_8__seal-halo" filter={`url(#${gid('bloom')})`}>
              ...
              <g className="poiv3_8__seal-core">
                {[geo.sealOuterRail, geo.sealInnerRail].map((radius) => (
                  <path
          ...
          {/* MEDALLION BODY */}
          <g className="poiv3_8__medal" clipPath="url(#c-medal)">
            {/* L1: Bronze outer body + texture + bevel */}
            <circle cx={geo.center} cy={geo.center} r={geo.medalRadius} fill={ringDark} />
            <circle className="poiv3_8__body" cx={geo.center} cy={geo.center} r={geo.medalRadius} fill={`url(#${gid('g-b')})`} filter="url(#f-ring-grain)" opacity=".92" />
            <circle className="poiv3_8__bevel" cx={geo.center} cy={geo.center} r={geo.medalRadius} fill={`url(#${gid('g-bv')})`} filter={isDragging ? undefined : "url(#f-dp)"} opacity=".48" />

            {/* L0.5: Milled outer rim — skipped in compact mode */}
            {!isCompact && (
              <>
                <circle
            ...
            {/* L0.8: Metal glints on the ring — skipped in compact mode */}
            {!isCompact && (
              <g className="poiv3_8__glints">
            ...
            {/* L1.5: Outer glaze lip — skipped in compact mode */}
            {!isCompact && (
              <circle
                className="poiv3_8__glaze"
                cx={geo.center}
                cy={geo.center}
                r={geo.glazeRadius}
                fill="none"
            ...
            {/* L4.5: Engraved rim script — dampened to texture; skipped in compact mode */}
            {!isCompact && (
            <g className="poiv3_8__rim">
              {rimBand.map((g, i) => (
                <g
                  key={`r-${i}`}
                  transform={`translate(${g.x.toFixed(2)} ${g.y.toFixed(2)}) rotate(${g.a})`}
                  opacity={T8.rune.baseOpacity + T8.rune.litBoost * g.lit}
                >
                  <g transform="scale(0.66)">
                    ...
                    {g.lit <= 0.01 && (
                      <path
                        ...
                        fill="none"
                        stroke={ringLight}
                        strokeOpacity={T8.rune.lipOpacity}
                        strokeWidth={1.4}
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        vectorEffect="non-scaling-stroke"
            ...
            {/* L5: Semantic glyph — one denotative sign per POI type, engraved look */}
            <g className="poiv3_8__cross" transform={`translate(43 43) scale(${T8.glyph.scale})`}>
              {/* Shadow pass — the cut into the metal */}
              <path
                d={T8.semanticIcons[type]}
                fill="none"
                stroke="rgba(0,0,0,0.6)"
                strokeWidth={T8.glyph.strokeWidth + 0.7}
                strokeLinecap="round"
                strokeLinejoin="round"
                transform={`translate(${T8.glyph.shadowOffset} ${T8.glyph.shadowOffset + 0.3})`}
              />
              {/* Main pass — catches the type rim light */}
              <path
                d={T8.semanticIcons[type]}
                fill="none"
                stroke={ringLight}
                strokeWidth={T8.glyph.strokeWidth}
                strokeLinecap="round"
                strokeLinejoin="round"
                filter={isDragging ? undefined : 'url(#f-glow)'}
              />
              {/* Top lip of the incision */}
              <path
                d={T8.semanticIcons[type]}
                fill="none"
                stroke={ringLight}
                ...
                strokeLinecap="round"
                strokeLinejoin="round"
                transform={`translate(${T8.glyph.highlightOffset} ${T8.glyph.highlightOffset})`}
              />
            </g>
            ...
            {/* L9: State channel — visuals that do not rely on colour alone */}
            {state === 'expiring' && (
              <path
                d={T8.state.crackPath}
                fill="none"
                stroke={`rgba(0,0,0,${T8.state.crackOpacity})`}
                strokeWidth={T8.state.crackStrokeWidth}
                strokeLinecap="round"
                strokeLinejoin="round"
              />
            )}
            {state === 'assigned' && (
              <circle
                cx="43" cy="43"
                r={T8.state.assignedSealRadius}
                fill="rgba(6,8,10,1)"
                opacity={T8.state.assignedSealOpacity}
              />
            )}
          </g>

          {/* Ground anchor — the badge is planted on the map, not floating */}
          {showAnchor && (
            <>
              <rect
                x={geo.center - T8.anchor.stemWidth / 2}
                y={T8.anchor.stemTopY}
                width={T8.anchor.stemWidth}
                height={T8.anchor.stemLength}
                rx={T8.anchor.stemWidth / 2}
                fill={ringDark}
                stroke={ringMid}
                strokeWidth="0.6"
              />
              <ellipse
                cx={geo.center}
                cy={T8.anchor.shadowCy}
                rx={T8.anchor.shadowRx}
                ry={T8.anchor.shadowRy}
                fill={`rgba(0,0,0,${T8.anchor.shadowOpacity})`}
              />
            </>
          )}

          {/* Threat channel — pulsing ring for critical importance */}
          {importance === 'critical' && (
            <circle
              className="poiv3_8__threat-ring"
              cx={geo.center}
              cy={geo.center}
              r={geo.medalRadius + 2.5}
              fill="none"
              stroke={T8.state.threatRingColor}
              strokeWidth={T8.state.threatRingWidth}
              strokeOpacity={T8.state.threatRingOpacity}
            />
          )}
        </svg>
      </div>
      ...
      <div className="poiv3_8__gem" style={{ position: 'absolute', left: '50%', bottom: '-14px', transform: 'translateX(-50%)', zIndex: 5, pointerEvents: 'none' }}>
...
export const poiMatericV3_8Styles = `
.poiv3_8 {
  position: relative;
  display: inline-flex;
  cursor: pointer;
...
.poiv3_8__svg { position: relative; width: 100%; height: 100%; overflow: visible; z-index: 4; }
.poiv3_8__seal { filter: url(#f-glow); opacity: 0.85; }
.poiv3_8__medal { filter: url(#f-glow); }
.poiv3_8--burst .poiv3_8__seal { opacity: 1; filter: url(#f-glow) drop-shadow(0 0 24px rgba(var(--poi-accent-rgb),0.55)); }
.poiv3_8--burst .poiv3_8__medal { filter: url(#f-glow) drop-shadow(0 0 28px rgba(var(--poi-accent-rgb),0.35)); }
.poiv3_8__body { }
.poiv3_8__bevel { transition: filter 250ms ease; }
.poiv3_8__glints { pointer-events: none; }
.poiv3_8:hover .poiv3_8__bevel { filter: brightness(1.2); }
.poiv3_8__cross { transition: filter 250ms ease; }
.poiv3_8:hover .poiv3_8__cross { filter: contrast(1.15) brightness(1.15); }
.poiv3_8__glass { transition: opacity 250ms ease; }
.poiv3_8:hover .poiv3_8__glass { opacity: 1; }
.poiv3_8__gem { transition: transform 250ms ease, filter 250ms ease; }
.poiv3_8:hover .poiv3_8__gem { transform: translateX(-50%) scale(1.08); filter: brightness(1.15); }
.poiv3_8__specular { transition: transform 80ms ease-out; }
.poiv3_8__vignette {
  position: absolute;
  left: 50%;
  top: 50%;
...
.poiv3_8__fog {
  ...
  background: radial-gradient(circle at 50% 46%,
    rgba(var(--poi-accent-rgb),0.05) 0%,
    transparent 60%
  );
}
.poiv3_8__threat-ring { animation: poiv3_8-threat-pulse 1.6s ease-in-out infinite; }
@keyframes poiv3_8-threat-pulse {
  0%, 100% { opacity: 0.25; }
  50% { opacity: 0.75; }
}
`;
