...
const QuestChronicle: React.FC<QuestChronicleProps> = ({
  ...
  const derivedVerbCards: DerivedPhaseCard[] = useMemo(() => {
    return phases.map((entry, index) => {
      ...
      const variantMap: Record<QuestPhaseType, 'amethyst' | 'ember' | 'jade'> = {
        check: 'amethyst',
        fight: 'ember',
        stealth: 'amethyst',
        trap: 'jade',
      ...
      const fallbackRisk = RISK_FALLBACKS[phase.type];
      const injuryPercentage = riskProfile?.injuryChance ?? fallbackRisk.injury;
      const deathPercentage = riskProfile?.deathChance ?? fallbackRisk.death;
  ...
  const activeNarrative = derivedVerbCards[currentPhaseIndex]?.phase.copy?.narrative ?? null;
...
const QuestPhaseSlot: React.FC<QuestPhaseSlotProps> = ({ card }) => {
  ...
  const icon = card.icon ?? (card.phase.type === 'fight' ? '⚔️' : card.phase.type === 'check' ? '🎲' : '🛠️');
  ...
  return (
    <div className="flex flex-1 flex-col items-center gap-1 pb-1">
      ...
      <div className="text-[10px] uppercase tracking-[0.3em] text-slate-200">{card.phase.title}</div>
    </div>
  );
};
...
const PhaseProgressSegment: React.FC<PhaseProgressSegmentProps> = ({ card, isLast }) => {
  ...
  return (
    <div className={clsx('relative flex-1 border-r border-slate-800/60', isLast && 'border-r-0!')}>
      <div
        className={clsx('absolute inset-y-0 left-0 rounded-r-full bg-linear-to-r shadow-[0_0_15px_rgba(251,191,36,0.35)]', fillClass)}
