...
import type { ActiveHUDSkinConfig } from '@/ui/idleVillage/skins/activeHUDSkinConfig';
import { createActiveHUDSkinConfig } from '@/ui/idleVillage/skins/activeHUDSkinConfig';
import { DEFAULT_HUD_PERSISTENCE_STATE } from '../utils/hudPersistence';
...
export interface ActiveHUDProps {
  ...
  skinConfig?: Partial<ActiveHUDSkinConfig>;
}
...
const ActiveHUD: React.FC<ActiveHUDProps> = ({
  ...
  const config = createActiveHUDSkinConfig(skinPresetId, pillar);
  const finalConfig: ActiveHUDSkinConfig = { ...config, ...skinConfig };

  // Emit telemetry event when skin is rendered
  useEffect(() => {
    trackTelemetryEvent('active_hud_value_change', {
      skinPresetId,
      pillar,
      componentTheme: finalConfig.componentTheme,
      valueChangeAnimation: finalConfig.valueChangeConfig.animationType,
      variant,
      maxVisible,
      activeCount: hudState?.activities?.length || 0,
      hasCrewHUD: !!crewHUDState,
    });
  }, [
    skinPresetId,
    pillar,
    finalConfig.componentTheme,
    finalConfig.valueChangeConfig.animationType,
    variant,
    maxVisible,
  ...
  const cssVariables = useMemo(() => {
    ...
    // Color variables
    vars['--hud-background'] = `var(${finalConfig.colors.backgroundToken})`;
    vars['--hud-border'] = `var(${finalConfig.colors.borderToken})`;
    vars['--hud-text'] = `var(${finalConfig.colors.textToken})`;
    vars['--hud-accent'] = `var(${finalConfig.colors.accentToken})`;
    vars['--hud-success'] = `var(${finalConfig.colors.successToken})`;
    vars['--hud-warning'] = `var(${finalConfig.colors.warningToken})`;
    vars['--hud-error'] = `var(${finalConfig.colors.errorToken})`;
    
    // Typography variables
    vars['--hud-title-font'] = `var(${finalConfig.typography.titleFont})`;
    vars['--hud-label-font'] = `var(${finalConfig.typography.labelFont})`;
    vars['--hud-value-font'] = `var(${finalConfig.typography.valueFont})`;
    vars['--hud-caption-font'] = `var(${finalConfig.typography.captionFont})`;
    
    // Spacing variables
    vars['--hud-container-padding'] = `var(${finalConfig.spacing.containerPadding})`;
    vars['--hud-item-spacing'] = `var(${finalConfig.spacing.itemSpacing})`;
    vars['--hud-section-spacing'] = `var(${finalConfig.spacing.sectionSpacing})`;
    
    // Effects variables
    vars['--hud-glow'] = `var(${finalConfig.effects.glowToken})`;
    vars['--hud-shadow'] = `var(${finalConfig.effects.shadowToken})`;
    vars['--hud-blur'] = `var(${finalConfig.effects.blurToken})`;
  ...
  useActivityTelemetry({
    hudState: hudState ?? { activities: [], counts: { jobs: 0, quests: 0, maintenance: 0, total: 0 }, hasActiveActivities: false, persistence: DEFAULT_HUD_PERSISTENCE_STATE },
