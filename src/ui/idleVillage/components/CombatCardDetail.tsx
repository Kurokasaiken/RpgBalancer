...
const CombatCardDetail: React.FC<CombatCardDetailProps> = ({
  ...
  const rafDelayRef = useRef<number | null>(null);
  ...
  useEffect(
    () => () => {
      if (rafDelayRef.current !== null) {
        cancelAnimationFrame(rafDelayRef.current);
        rafDelayRef.current = null;
      }
    },
    [],
  );

  const waitForFrameDuration = useCallback((durationMs: number) => {
    if (rafDelayRef.current !== null) {
      cancelAnimationFrame(rafDelayRef.current);
      rafDelayRef.current = null;
    }
    ...
    return new Promise<void>((resolve) => {
      ...
      const step = (timestamp: number) => {
        ...
        rafDelayRef.current = requestAnimationFrame(step);
      };
  ...
  const handleStartCombat = useCallback(async () => {
    ...
    try {
      ...
      for (const entry of logEntries) {
        ...
        await waitForFrameDuration(800);
      }
  ...
  }, [onStartCombat, onCombatComplete, phase.id, isCombatActive, waitForFrameDuration]);
        ...
        <div className="relative z-10 flex flex-col gap-3">
          <header className="flex items-start justify-between gap-2.5">
            <div className="flex-1 space-y-0.5">
              <h2 className="text-sm font-semibold leading-tight tracking-wide">{phase.title}</h2>
              <p className="text-[9px] uppercase tracking-[0.15em] text-amber-200/80">{questTitle}</p>
            </div>
