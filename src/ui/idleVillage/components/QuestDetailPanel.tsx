...
export const QuestDetailPanel: React.FC<QuestDetailPanelProps> = ({ summary, activity, config }) => {
  ...
  const riskMetrics = deriveTheaterRiskStripes({
    injuryPercentage: summary.injuryPercentage ?? 0,
    deathPercentage: summary.deathPercentage ?? 0,
  });
  const questEntityId = summary.slotId ?? activity.id;
  const { entries: questModifiers, isLoading: questModifiersLoading } = useModifierVisualization('questDetail', {
  ...
  return (
    <section className="rounded-3xl border border-amber-400/40 bg-black/75 p-4 shadow-[0_18px_60px_rgba(0,0,0,0.65)] backdrop-blur">
      ...
      <div className="mt-4 flex items-center gap-4">
        <div
          className="relative flex h-20 w-3 flex-col overflow-hidden rounded-full border border-slate-800 bg-slate-950/70 shadow-inner"
          data-testid="quest-detail-risk-stripe"
        ...
        >
          ...
          {riskMetrics.segments.safeHeightPercent > 0 && (
            <div
              className="flex-1 bg-emerald-800/30"
      ...
      {(questModifiersLoading || questModifiers.length > 0) && (
        <div className="mt-4">
          <div className="text-[10px] uppercase tracking-[0.3em] text-slate-400 mb-1">Modifier preview</div>
          <StatModifierDisplay
            modifierEntries={questModifiers}
            isLoading={questModifiersLoading}
            showHeader={false}
            maxVisible={5}
            emptyLabel="Nessun modificatore applicato alla quest"
            testId={`quest-${questEntityId}-modifier-display`}
          />
        </div>
      )}
