...
import {
  solveCorolla as scSolveCorolla,
  tstOf as scTstOf,
...
export function createDestinyAstrolabeV10Engine(root: HTMLElement, opts: AstrolabeEngineOpts): AstrolabeEngineHandle {
  console.log('[engine] createDestinyAstrolabeV10Engine called, skills=', opts.skills?.map(s=>`${s.name}:${s.stat}/${s.difficulty}`));
...
function snap(){
  ...
  /* ══ V10 — LA COROLLA RISOLTA ═══════════════════════════════════════════
     Unica differenza strutturale dalla V7, ed e' un'inversione di direzione.
     Nella V7 la geometria E' la regola: si misura l'area e quella e' la
     probabilita'. Qui la probabilita' e' una REGOLA DI GIOCO (tst) e la FORMA
     viene risolta per illustrarla esattamente.
     Il guadagno: il corpo del fiore non puo' piu' essere piu' grande dell'area
     di gioco, perche' il termine a cui tende nella fusione E' il muro. Niente
     da clippare. Il prezzo: se il disegno sbaglia, il board mente — nella V7
     non poteva, perche' il disegno era la regola. */
  const input={stats:Array.from({length:AXES},(_,i)=>pick(i).stat),
               diffs:Array.from({length:AXES},(_,i)=>pick(i).difficulty)};
  const conf={crit:cfg.crit, critWin:cfg.critWin, almost:cfg.almostPct,
              death:cfg.dead, wound:cfg.wound};
  const statMean=input.stats.reduce((a,b)=>a+b,0)/AXES;
  const diffMean=input.diffs.reduce((a,b)=>a+b,0)/AXES;
  const shape=scSolveCorolla(input, conf, scTstOf(statMean, diffMean, cfg.crit));
  snapCache=scBuildSnapshot(input, conf, 0, shape);
  return snapCache;
}
...
const scene={
  ...
  scrim:null,                           // {x,y,t} — lo scrim col buco sul punto d'arresto
...
function launchRoll(){
  ...
  scene.quake=null; scene.scrim=null;
...
function tickTimeline(){
  ...
  if(scene.scrim) scene.scrim.t+=16.7;
...
function drawWebLayer(){
  ...
  drawWeb(ctx, {
    ...
    /* LA TELA E' GRANDE COME IL BOARD E APPESA ALLA GHIERA (desiderata v13).
       ...
       legata dai tiranti. */
    /* V10: la tela torna a coincidere con l'ARENA. Sta sotto il fiore, e con la
       corolla il fiore non la supera piu': quindi non serve piu' che sia grande
       come il board per non essere coperta. */
...
/* ══ GLI AGHI — la misura della stat, separata dall'area ═══════════════════
   Il fiore faceva due lavori: misurare la stat (punte a rOf(stat)) ed essere
   l'area di successo, in conflitto ogni volta che stat > difficolta'. Qui la
   misura la portano cinque aghi larghi due pixel: possono uscire dal muro
   quanto vogliono, perche' non coprono niente. */
function drawNeedles(){
  ...
  ctx.lineCap='round';
  for(let i=0;i<AXES;i+=1){
    const a=TIP(i), r=geo.axisTip[i]*s;
    const x0=CX+Math.cos(a)*geo.rCrit*0.9, y0=CY+Math.sin(a)*geo.rCrit*0.9;
    const x1=CX+Math.cos(a)*r,             y1=CY+Math.sin(a)*r;
    ctx.lineWidth=3.4; ctx.strokeStyle='rgba(6,10,16,0.7)';
    ctx.beginPath(); ctx.moveTo(x0,y0); ctx.lineTo(x1,y1); ctx.stroke();
    ctx.lineWidth=1.6; ctx.strokeStyle='rgba(255,232,168,0.95)';
    ctx.shadowColor='rgba(255,214,130,0.8)'; ctx.shadowBlur=8;
    ctx.beginPath(); ctx.moveTo(x0,y0); ctx.lineTo(x1,y1); ctx.stroke();
    ctx.shadowBlur=0;
    /* la capocchia tocca il righello: e' lei che si legge come "85" */
    ctx.beginPath(); ctx.arc(x1,y1,3.2,0,TAU);
    ctx.fillStyle='rgba(255,244,206,1)'; ctx.fill();
    ctx.lineWidth=1; ctx.strokeStyle='rgba(120,80,20,0.8)'; ctx.stroke();
  }
  ctx.restore();
}
...
/* SCRIM DELLA RISOLUZIONE — al posto del filtro CSS che spegneva il canvas.
   La periferia si spegne, il punto d'arresto resta a piena luminosita': si
   spegne INTORNO a cio' che va guardato, non sopra. */
function drawResolutionScrim(){
  const S2=scene.scrim;
  const R2=scene.res;
  /* timer PROPRIO: appoggiarsi a `scene.impact.t` faceva sparire lo scrim dopo
     560ms, quando la pulsazione d'impatto viene liberata. Lo scrim deve durare
     quanto la risoluzione. */
  if(!R2||!S2) return;
  const isLoss=(R2.verdict==='fail'||R2.verdict==='epicfail');
  const p=clamp(S2.t/420,0,1)*(isLoss?(R2.verdict==='epicfail'?0.92:0.72):0.42);
  if(p<=0.001) return;
  const g=ctx.createRadialGradient(S2.x,S2.y,40,S2.x,S2.y,R*1.05);
  g.addColorStop(0,'rgba(2,4,12,0)');
  g.addColorStop(0.22,`rgba(2,4,12,${(0.42*p).toFixed(3)})`);
  g.addColorStop(1,`rgba(2,4,12,${(0.80*p).toFixed(3)})`);
  ctx.fillStyle=g;
...
function drawBall(now){
  ...
  ctx.restore();
  /* IL PUNTO DI CONTATTO. Misurato: il disco della pallina (18px) e' da 2 a 7
     volte lo spessore della fascia del fallimento critico (2.6-8.5px secondo la
     difficolta'), quindi quando "si ferma nel critico" copre critico, fail e un
     pezzo di stella insieme: la lettura e' ambigua per costruzione.
     La pallina resta grande — e' il protagonista — ma cio' che il board LEGGE e'
     questo marcatore da 3px, che dentro una fascia da 4.85px ci sta.
     Il disco dice "guardami", il marcatore dice "sono qui". */
  if(fallK<=0){
    ctx.beginPath(); ctx.arc(b.x,b.y,3,0,TAU);
    ctx.fillStyle='rgba(8,6,14,0.95)'; ctx.fill();
    ctx.lineWidth=1; ctx.strokeStyle='rgba(255,236,180,0.95)'; ctx.stroke();
  }
  ctx.shadowBlur=0;
}
...
function frame(now){
 try{
  ...
  drawAxisRig(now);
  drawBlueprint();
  /* ══ V10 — LA TELA VA SOTTO (prima), come nelle versioni precedenti ═══════
     Non e' un velo addosso al fiore: e' la superficie su cui il fiore poggia.
     E funziona perche' con la corolla il corpo non puo' piu' essere piu' grande
     dell'area di gioco — che era la ragione per cui la tela era stata spostata
     sopra: sotto, non si vedeva. */
  drawWebLayer();
  drawStar(now);
  /* gli AGHI sopra tutto: sono la misura, e devono restare leggibili */
  drawNeedles();
  decayWardFlashes();
