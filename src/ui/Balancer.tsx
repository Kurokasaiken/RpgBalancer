...
export const Balancer: React.FC = () => {
    ...
    const [activePresetId, setActivePresetIdState] = useState<string>('standard');
    const [currentPreset, setCurrentPreset] = useState<BalancePreset | null>(null);
    ...
    useEffect(() => {
        ...
        const loadActivePreset = async () => {
            try {
                const preset = await BalanceConfigManager.getActivePreset();
                if (!isMounted) {
                    return;
                }
    ...
    const handlePresetChange = async (id: string): Promise<void> => {
        try {
            await BalanceConfigManager.setPreset(id);
            const updatedPreset = await BalanceConfigManager.getActivePreset();
            setActivePresetIdState(updatedPreset.id);
            setCurrentPreset(updatedPreset);

            // Trigger recalculation with new weights
            setStats(prev => BalancingSolver.recalculate(prev));
            toast.success(`Preset switched to ${updatedPreset.name}`);
        } catch (error) {
    ...
    const handleWeightEditorSave = async (newPresetId: string): Promise<void> => {
        try {
            await handlePresetChange(newPresetId);
        } catch (error) {
            // handlePresetChange already logged and toasted, but WeightEditor callers expect rejections handled
            console.warn('Weight editor save failed', error);
        }
    };
    ...
    return (
        <div className="h-full overflow-y-auto bg-gradient-to-br from-green-950 via-emerald-950 to-slate-950 p-4 relative">
            ...
            {/* Weight Editor Modal */}
            {showWeightEditor && currentPreset && (
                <WeightEditor
