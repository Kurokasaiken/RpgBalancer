...
type ThumbnailScale = 'xs' | 'sm' | 'md';

const THUMBNAIL_SCALES: ThumbnailScale[] = ['xs', 'sm', 'md'];

const THUMBNAIL_SCALE_LABELS: Record<ThumbnailScale, string> = {
  xs: 'XS',
  sm: 'SM',
  md: 'MD',
};
...
/**
 * Attempts to read an ISO-like date (YYYY-MM-DD or YYYYMMDD) from the provided string.
 */
const extractIsoDateTimestamp = (value: string): number | null => {
  const isoMatch = value.match(/(20\d{2})[-_]?(\d{2})[-_]?(\d{2})/);
...
/**
 * Builds a numeric fingerprint from any digit sequence contained inside the string.
 * Useful as a fallback ordering heuristic when no explicit date is present.
 */
const extractNumericFingerprint = (value: string): number | null => {
  const digits = value.match(/\d+/g);
  if (!digits) return null;
  const fingerprint = digits.join('').slice(-12);
  const numericValue = Number(fingerprint);
  return Number.isNaN(numericValue) ? null : numericValue;
};

/**
 * Derives a deterministic sort key so the UI can approximate "most recently added" images.
 */
const computeImageSortKey = (id: string, fileName: string): number => {
  const isoDate = extractIsoDateTimestamp(fileName) ?? extractIsoDateTimestamp(id);
  if (isoDate) return isoDate;
  const numericFingerprint = extractNumericFingerprint(fileName) ?? extractNumericFingerprint(id);
  if (numericFingerprint) return numericFingerprint;
  return 0;
};
...
const MOOD_IMAGES: MoodImage[] = Object.entries(moodImageModules)
  .map(([path, mod]) => {
    ...
    const sortKey = computeImageSortKey(normalizedPath, fileName);
    const image: MoodImage = {
      id: normalizedPath,
      src: mod.default,
      fileName,
      folder,
...
export function MoodboardPage() {
  ...
  const [thumbnailScale, setThumbnailScale] = useState<ThumbnailScale>('xs');
  ...
  const folderOptions = useMemo(() => {
    const uniqueFolders = new Set(images.map((image) => image.folder));
    return ['all', ...Array.from(uniqueFolders)];
  }, [images]);

  const filteredThumbnails = useMemo(() => {
    if (folderFilter === 'all') return images;
    return images.filter((image) => image.folder === folderFilter);
  }, [folderFilter, images]);
  ...
  const goDirectlyToImage = useCallback(
    (imageId: string) => {
      const targetIndex = imageIndexMap[imageId];
  ...
  const handleFolderChange = useCallback((event: ChangeEvent<HTMLSelectElement>) => {
    setFolderFilter(event.target.value);
  }, []);

  const handleThumbnailScaleChange = useCallback((scale: ThumbnailScale) => {
  ...
  const contactSheetGridClass = useMemo(
    () => `moodboard-thumb-grid moodboard-thumb-grid--${thumbnailScale}`,
    [thumbnailScale],
  );
  ...
  return (
    <TooltipProvider testId="moodboard-tooltip-provider">
      <div className="moodboard-shell" style={shellStyle} data-testid="moodboard-content" data-moodboard="true">
      <div className="moodboard-shell__grid" onKeyDown={handleKeyboard} tabIndex={0}>
        <div className="moodboard-main">
          ...
          <section className="moodboard-panel moodboard-contact-panel">
            <div className="moodboard-contact-header">
              <div>
                <p className="moodboard-panel__kicker">Contact Sheet</p>
                <p className="moodboard-panel__subtitle">
                  {filteredThumbnails.length} asset · Ordine: caricamenti più recenti
                </p>
              </div>
              <button type="button" className="moodboard-button" onClick={handleToggleContactSheet}>
                {isContactSheetOpen ? 'Nascondi griglia' : 'Mostra griglia'}
              </button>
            </div>

            <div className="moodboard-contact-controls">
              <label className="moodboard-contact-label">
                Cartella
                <select className="moodboard-contact-select" value={folderFilter} onChange={handleFolderChange}>
                  {folderOptions.map((folder) => (
                    <option key={folder} value={folder}>
                      {folder === 'all' ? 'Tutte le cartelle' : folder}
                    </option>
                  ))}
                </select>
              </label>

              <div className="moodboard-thumb-scales" role="group" aria-label="Thumbnail scale">
                {THUMBNAIL_SCALES.map((scale) => (
                  <button
                    key={scale}
                    type="button"
                    className={`moodboard-thumb-scale ${thumbnailScale === scale ? 'is-active' : ''}`}
                    onClick={() => handleThumbnailScaleChange(scale)}
                  >
                    {THUMBNAIL_SCALE_LABELS[scale]}
                  </button>
                ))}
              </div>
            </div>
            ...
            {isContactSheetOpen ? (
              filteredThumbnails.length > 0 ? (
                  ...
                  {filteredThumbnails.map((image) => {
                    ...
                    return (
                      <button
                        key={image.id}
                        type="button"
                        className={`moodboard-thumb ${isActive ? 'is-active' : ''}`}
                        onClick={() => goDirectlyToImage(image.id)}
                        aria-label={`Mostra ${image.fileName}`}
                      >
                        <img src={image.src} alt={image.fileName} loading="lazy" />
                        <span className="moodboard-thumb__meta">
                          <span className="moodboard-thumb__file" title={image.fileName}>
                            {image.fileName}
                          </span>
                          <span className="moodboard-thumb__folder">{image.folder}</span>
                        </span>
                      </button>
                    );
                  })}
                ...
                <p className="moodboard-contact-empty">Nessun asset nella cartella selezionata.</p>
              )
            ) : (
              <p className="moodboard-contact-collapsed">Griglia nascosta · usa il pulsante per riaprirla.</p>
            )}
          </section>
