...
import { CharacterPortraitPicker } from './components/CharacterPortraitPicker';
...
export const CharacterCreator: React.FC = () => {
    ...
    const archetypes = useMemo<ArchetypeTemplate[]>(() => {
        const registry = new ArchetypeRegistry();
        return registry.listAll();
    }, []);
    const loadingArchetypes = false;
    ...
    const handleSpellsChange = (spells: Character['equippedSpells']) => {
        setCharacter(prev => ({ ...prev, equippedSpells: spells }));
    };

    const handleVisualProfileChange = (profileId: string) => {
        setCharacter(prev => ({
            ...prev,
    ...
    const handlePortraitUrlChange = (url: string) => {
        const sanitized = url.trim();
        setCharacter(prev => ({
            ...prev,
            portraitUrl: sanitized.length > 0 ? sanitized : undefined,
        }));
    };

    const handleFullFigureUrlChange = (url: string) => {
        const sanitized = url.trim();
        setCharacter(prev => ({
            ...prev,
            fullFigureUrl: sanitized.length > 0 ? sanitized : undefined,
        }));
    };
    const handlePortraitCropChange = (crop: PortraitCropSettings) => {
        setCharacter(prev => ({
            ...prev,
            portraitCrop: crop,
        }));
    ...
    const handleSave = () => {
        ...
        const finalCharacter = CharacterBuilder.buildCharacter(buildParams());
        ...
        input.onchange = (e) => {
            ...
            reader.onload = (event) => {
                try {
                    ...
                    setCustomWeights(imported.customWeights ?? {});
    ...
    const previewCharacter = {
        ...
        pointBudget,
        statAllocations,
        stats: CharacterBuilder.buildCharacter(buildParams()).stats
    };
    ...
    return (
        <div className="h-full overflow-y-auto bg-linear-to-br from-indigo-950 via-purple-950 to-slate-950 p-4 relative">
            ...
            <div className="max-w-7xl mx-auto relative z-10">
                ...
                {/* Visual separator */}
                <div className="h-px bg-linear-to-r from-transparent via-white/20 to-transparent my-6" />
                        ...
                        <div className="fantasy-glass rounded-lg p-4 space-y-3">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    onFullFigureUrlChange={handleFullFigureUrlChange}
                                    onPortraitCropChange={handlePortraitCropChange}
                                />
                            </div>
                ...
                <div className="flex flex-wrap gap-3 justify-center">
                    <button
                        onClick={handleSave}
                        className="px-6 py-3 bg-linear-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 text-white font-bold rounded-lg transition-all fantasy-hover-glow-nature"
                    >
                        💾 Save Character
                    </button>
                    ...
                    <button
                        ...
                        className="px-6 py-3 bg-linear-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white font-bold rounded-lg transition-all fantasy-hover-glow-water"
                    >
                        📥 Export JSON
                    </button>
                    ...
                    <button
                        ...
                        className="px-6 py-3 bg-linear-to-r from-purple-600 to-violet-600 hover:from-purple-500 hover:to-violet-500 text-white font-bold rounded-lg transition-all fantasy-hover-glow-arcane"
                    >
                        📤 Import JSON
                    </button>
