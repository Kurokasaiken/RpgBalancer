...
export const StatStressTestingPage: React.FC = () => {
  ...
  const hasResults = Boolean(currentResults?.efficiencies?.length);
  ...
  const exportJSON = useCallback(() => {
    const data = aggregatedResults || currentResults;
    if (!data) return;
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `stat-efficiency-${Date.now()}.json`;
    a.click();
    URL.revokeObjectURL(url);
  }, [aggregatedResults, currentResults]);

  const exportCSV = useCallback(() => {
    const data = currentResults?.efficiencies;
    if (!data) return;
    const headers = ['Rank', 'StatId', 'Efficiency', 'Wins', 'Losses', 'Draws', 'Assessment'];
    const rows = data.map(e => [e.rank, e.statId, e.efficiency, e.wins, e.losses, e.draws, e.assessment]);
    const csv = [headers, ...rows].map(row => row.join(',')).join('\n');
    const blob = new Blob([csv], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `stat-efficiency-${Date.now()}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  }, [currentResults]);
  ...
  const exportMarkdown = useCallback(() => {
    const data = currentResults?.efficiencies;
    if (!data) return;
    const headers = ['Rank', 'Stat', 'Efficiency', 'W/L', 'Assessment'];
    const rows = data.map(e => [e.rank, e.statId, e.efficiency.toFixed(3), `${e.wins}/${e.losses}`, e.assessment]);
    const md = [
      '| ' + headers.join(' | ') + ' |',
      '| ' + headers.map(() => '---').join(' | ') + ' |',
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `stat-efficiency-${Date.now()}.md`;
    a.click();
    URL.revokeObjectURL(url);
  }, [currentResults]);
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,#1f2937_0,#020617_55%,#000000_100%)] text-slate-200 px-4 py-6">
      <div className="max-w-6xl mx-auto space-y-4 relative">
        ...
        <div className="pointer-events-none absolute inset-0 opacity-40 -z-10">
          <div className="absolute -top-32 -left-32 w-72 h-72 rounded-full bg-[radial-gradient(circle,rgba(129,140,248,0.35)_0,transparent_60%)] blur-3xl animate-pulse" />
          <div className="absolute -bottom-24 -right-16 w-80 h-80 rounded-full bg-[radial-gradient(circle,rgba(56,189,248,0.3)_0,transparent_65%)] blur-3xl animate-[ping_12s_linear_infinite]" />
        </div>
          ...
          <div className="flex flex-col items-end gap-2 min-w-[260px]">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-full border border-cyan-400/70 text-cyan-200 text-[10px] tracking-[0.32em] uppercase bg-slate-900/60 transition disabled:cursor-not-allowed disabled:opacity-60 ${
                  isRunning
                    ? 'shadow-[0_0_22px_rgba(34,211,238,0.8)] animate-pulse'
                    : 'hover:bg-cyan-500/10 hover:-translate-y-px'
                }`}
              >
                {isRunning && (
                  <span className="inline-block w-4 h-4 border-2 border-cyan-300/80 border-t-transparent rounded-full animate-spin" />
                )}
              ...
              >
                {isAutoBalancing && (
                  <span className="inline-block w-4 h-4 border-2 border-amber-300/80 border-t-transparent rounded-full animate-spin" />
                )}
              ...
              {hasResults && (
                <div className="flex gap-2">
                  <button
                    type="button"
                    onClick={exportJSON}
                    className="inline-flex items-center gap-2 px-3 py-2 rounded-full border border-emerald-400/70 text-emerald-200 text-[10px] tracking-[0.32em] uppercase bg-slate-900/60 hover:bg-emerald-500/10"
                  >
                    Export JSON
                  </button>
                  <button
