...
type PremiumCardRarity = 'common' | 'rare' | 'epic' | 'legendary';
...
> = {
  common: { glow: 'rgba(255,255,255,0.35)', border: 'from-slate-200 to-slate-400' },
  rare: { glow: 'rgba(14,165,233,0.6)', border: 'from-sky-400 to-blue-600' },
  epic: { glow: 'rgba(192,132,252,0.7)', border: 'from-purple-400 to-fuchsia-700' },
  legendary: { glow: 'rgba(249,115,22,0.8)', border: 'from-amber-400 to-orange-600' },
};

interface HearthstoneCardPremiumProps {
  name?: string;
  manaCost?: number;
  attack?: number;
  health?: number;
  rarity?: PremiumCardRarity;
  cardClass?: string;
  artwork?: string;
}

const PREMIUM_FRAME_URL = 'https://files.catbox.moe/9j7p4v.png';
const PREMIUM_GEM_URL = 'https://files.catbox.moe/5k8m3n.png';
const PREMIUM_CLASS_ICON_URL = 'https://files.catbox.moe/2q9v5x.png';
const PREMIUM_BACKGROUND_URL = 'https://files.catbox.moe/3l1k2q.png';

function HearthstoneCardPremium({
  ...
  rarity = 'legendary',
  cardClass = 'Warrior',
  artwork = 'https://files.catbox.moe/1p0w7y.jpg',
}: HearthstoneCardPremiumProps) {
  const rarityStyle = RARITY_STYLE_MAP[rarity];

  return (
    <div className="relative mx-auto flex h-[480px] w-[320px] flex-col items-center justify-center rounded-[32px] border border-amber-200/20 bg-[#05070f] p-4 shadow-[0_35px_100px_rgba(0,0,0,0.85)]">
      {/* Textured background */}
      <div
        className="absolute inset-0 rounded-[32px] opacity-90"
        style={{
          backgroundImage: `linear-gradient(180deg, rgba(10,12,20,0.9) 0%, rgba(5,7,15,0.95) 60%, rgba(3,4,8,1) 100%), url('${PREMIUM_BACKGROUND_URL}')`,
      ...
      <div
        ...
        style={{ boxShadow: `0 0 60px ${rarityStyle.glow}` }}
      />
      ...
      <div className="pointer-events-none absolute inset-0">
        {['top-4 left-4', 'top-4 right-4', 'bottom-4 left-4', 'bottom-4 right-4'].map((pos) => (
          <img
            ...
            src={PREMIUM_GEM_URL}
            alt=""
            className={`absolute ${pos} h-10 w-10 select-none drop-shadow-[0_0_12px_rgba(250,204,21,0.7)]`}
            loading="lazy"
          />
        ))}
      </div>
      ...
      <div
        className={[
          'absolute top-4 left-1/2 flex h-16 w-16 -translate-x-1/2 items-center justify-center rounded-full border-4 text-3xl font-black text-white shadow-[0_12px_24px_rgba(0,0,0,0.55)]',
          `bg-gradient-to-br ${rarityStyle.border}`,
          'border-amber-200/80',
        ].join(' ')}
      >
        {manaCost}
      ...
      {/* Class crest */}
      <img
        src={PREMIUM_CLASS_ICON_URL}
        alt={`${cardClass} crest`}
        className="absolute top-5 left-5 h-14 w-14 drop-shadow-[0_10px_25px_rgba(0,0,0,0.7)]"
        loading="lazy"
      />

      {/* Artwork */}
      <div className="absolute inset-x-8 top-24 bottom-32 overflow-hidden rounded-[22px] border border-amber-200/20 shadow-[0_15px_35px_rgba(0,0,0,0.55)]">
        <img src={artwork} alt={name} className="h-full w-full object-cover" loading="lazy" />
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-[rgba(3,4,8,0.95)]" />
      </div>

      {/* Nameplate */}
      <div className="absolute top-16 left-0 right-0 flex flex-col items-center gap-1 text-center">
        <span className="text-[22px] font-semibold uppercase tracking-[0.2em] text-amber-100 drop-shadow-[0_4px_16px_rgba(0,0,0,0.8)]">
          {name}
        </span>
        <span className="text-[10px] uppercase tracking-[0.4em] text-slate-300">{cardClass}</span>
      </div>

      {/* Stats */}
      <div className="absolute bottom-20 left-0 right-0 flex items-center justify-center gap-16">
        {[{ label: 'ATK', value: attack }, { label: 'HP', value: health }].map((stat) => (
          <div
            key={stat.label}
            className={[
              'relative flex h-20 w-20 flex-col items-center justify-center rounded-full border-4 text-4xl font-black text-amber-50 shadow-[0_10px_25px_rgba(0,0,0,0.65)]',
              `bg-gradient-to-br ${rarityStyle.border}`,
              'border-amber-200/70',
            ].join(' ')}
          >
            {stat.value}
            <span className="absolute -bottom-5 text-[10px] tracking-[0.4em] text-amber-200">{stat.label}</span>
          </div>
        ))}
      </div>
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden rounded-[32px]">
        <div className="absolute inset-y-0 left-[-60%] h-full w-1/2 rotate-12 bg-gradient-to-r from-transparent via-white/20 to-transparent blur-xl opacity-0 transition-opacity duration-500 hover:opacity-80" />
      </div>
    </div>
  );
}
