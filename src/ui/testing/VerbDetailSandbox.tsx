...
import VerbCard, { type VerbVisualVariant, type ProgressStyle } from '@/ui/idleVillage/legacy/VerbCard';
import MarbleCard from '@/ui/fantasy/assets/marble-verb-card/MarbleCard';
import MapMarker from '@/ui/idleVillage/legacy/MapMarker';
...
import VerbDetailCard, {
  ...
  type VerbSlotState,
} from '@/ui/idleVillage/legacy/VerbDetailCard';
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      if (slotId === 'verb_card_main') {
        ...
        // Cannot assign to null slotId anymore - this case should be handled by validation
        pushDebug(`Cannot assign resident ${residentId} to null slot`);
        return;
      }
          ...
          <div className="grid gap-5">
            ...
            <section className="default-card flex flex-col gap-4">
              ...
              <div className="rounded-2xl border border-slate-900/70 bg-[#050b0f]/80 p-3 shadow-inner shadow-black/60">
                <div className="flex items-center justify-between text-[9px] uppercase tracking-[0.25em] text-slate-400">
                  <span>Debug Log</span>
                  <span className={cardDropState === 'valid' ? 'text-emerald-300' : cardDropState === 'invalid' ? 'text-white/60' : 'text-slate-500'}>
