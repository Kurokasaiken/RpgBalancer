...
export function deriveAxisValues(
  stats: StatRow[],
  fallbackEnemy: readonly number[] = DEFAULT_AXIS_VALUES.enemy,
  fallbackPlayer: readonly number[] = DEFAULT_AXIS_VALUES.player,
  axes = 5,
): AxisValues {
...
/**
 * Generates randomized quest/hero stat pairs for a given axis count, leveraging the
 * {@link randomizeAxisValues} jitter logic so scenes can reshuffle values per run.
 */
export function generateRandomAxisStatPairs(
  axisCount: number,
  options?: AxisRandomizationOptions,
): AxisStatPair[] {
  const baseEnemy = Array.from({ length: axisCount }, (_, index) => DEFAULT_AXIS_VALUES.enemy[index % DEFAULT_AXIS_VALUES.enemy.length]);
  const basePlayer = Array.from({ length: axisCount }, (_, index) => DEFAULT_AXIS_VALUES.player[index % DEFAULT_AXIS_VALUES.player.length]);
  const randomized = randomizeAxisValues({ enemy: baseEnemy, player: basePlayer }, options);
  return randomized.enemy.map((questValue, index) => ({
    quest: questValue,
    hero: randomized.player[index] ?? randomized.player[randomized.player.length - 1] ?? questValue,
  }));
}
