...
export const FantasySpellCreation: React.FC = () => {
    ...
    const getStatSteps = (field: string) => statSteps[field] || [{ value: (spell as unknown as Record<string, unknown>)[field] || 0, weight: 1 }];

    const updateStatStep = (field: string, idx: number, step: { value: number; weight: number }) => {
        setStatSteps(prev => {
            const steps = [...(prev[field] || [{ value: (spell as unknown as Record<string, unknown>)[field] || 0, weight: 1 }])];
            steps[idx] = step;
    ...
    const handleSelectTick = (field: string, idx: number) => {
        ...
        const steps = statSteps[field] || [{ value: (spell as unknown as Record<string, unknown>)[field] || 0, weight: 1 }];
        if (steps[idx]) {
    ...
    const handleSave = () => {
        ...
        (Object.keys(spell) as (keyof Spell)[]).forEach((key) => {
            ...
            const defaultValue = (defaultSpell as unknown as Record<string, unknown>)[key];
            if (value !== undefined && value !== defaultValue) {
                (minimalSpell as unknown as Record<string, unknown>)[key] = value;
            }
        });
                        ...
                        <div className="backdrop-blur-md bg-cyan-900/20 border border-cyan-500/30 rounded-lg p-4 h-full shadow-[0_4px_16px_rgba(6,182,212,0.15)] overflow-y-auto">
                            ...
                            <ul className="text-xs text-cyan-50 grid grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-1">
                                ...
                                {Object.entries(spell)
                                    .filter(([key, value]) => {
                                        const defaultSpell = DEFAULT_SPELLS[0];
                                        return key !== 'id' && key !== 'name' && key !== 'type' && key !== 'effect' && key !== 'targetStat' && value !== undefined && value !== (defaultSpell as unknown as Record<string, unknown>)[key] && typeof value !== 'object';
                                    })
                                    .map(([key, value]) => (
