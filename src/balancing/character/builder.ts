...
import { DEFAULT_RESIDENT_VISUAL_PROFILE_ID } from '../config/idleVillage/residentVisuals';
...
export const CharacterBuilder = {
    ...
    buildCharacter: (params: CharacterCreationParams): Character => {
        ...
        const character: Character = {
            ...
            tags: params.tags,
            visualProfileId: params.visualProfileId ?? DEFAULT_RESIDENT_VISUAL_PROFILE_ID,
            portraitUrl: params.portraitUrl,
            fullFigureUrl: params.fullFigureUrl,
            portraitCrop: params.portraitCrop,
        };
        ...
        Object.entries(allocations).forEach(([statName, points]) => {
            if (points && points > 0) {
                const weight = typeof weights[statName] === 'number'
                    ...
                    : (weights[statName] as { avgRatio: number } | undefined)?.avgRatio || 1;
                ...
                stats = BalancingSolver.solve(
                    ...
                    (baseStats as unknown as Record<string, number>)[statName] + statValue,
                    'none'
                );
            }
        });
    ...
    createEmpty: (archetype: string = 'balanced'): Character => {
        return {
            ...
            createdAt: new Date(),
            modifiedAt: new Date(),
            visualProfileId: DEFAULT_RESIDENT_VISUAL_PROFILE_ID
        };
    },
