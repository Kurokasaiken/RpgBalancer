...
let BASE_CONFIG: BalancerConfig | null = null;

beforeAll(async () => {
  BASE_CONFIG = await BalancerConfigStore.load();
});

/**
 * Returns a deep-cloned config snapshot so tests do not mutate shared state.
 */
function freshConfig(): BalancerConfig {
  if (!BASE_CONFIG) {
    throw new Error('Balancer config not loaded; ensure beforeAll completed.');
  }
  return JSON.parse(JSON.stringify(BASE_CONFIG)) as BalancerConfig;
}

/**
 * Builds a simple stat-to-value map using each stat's configured default.
 */
...
/**
 * Helper assertion that actual and expected numbers are within an epsilon.
 */
