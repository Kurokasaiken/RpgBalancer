...
describe('BalancerConfigStore formula round-trip', () => {
  ...
  it('preserves custom derived stat formulas after export/import', async () => {
    const baseConfig = await BalancerConfigStore.load();
    ...
    await BalancerConfigStore.save(configWithDerived, 'added derived stat');
  ...
  it('re-hydrates missing default derived stats with formulas during import', async () => {
    const exported = await BalancerConfigStore.export();
    ...
    const imported = await BalancerConfigStore.import(reserialized);
