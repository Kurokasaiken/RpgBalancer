/**
 * Config for the Destiny Astrolabe V6.4 semantic/cinematic upgrade (R-067).
 *
 * Real clone of `astrolabeV63Config` — V6.4 owns its own token set so the
 * presentation can diverge without touching V6.3. Adds `phaseDurations`
 * (F0) so the engine timeline is fully config-driven instead of reading
 * scattered `cfg.t*` / hardcoded constants.
 */

import { z } from 'zod';

/** Zod schema for the V6.4 presentation contract. */
const astrolabeV64ConfigSchema = z.object({
  /** Centralized phase timings — the engine timeline source of truth. */
  phaseDurations: z.object({
    /** Technical beat before the threat slam (the bezel no longer animates). */
    ringMs: z.number().int().min(0).max(2000),
    /** Obelisk slam duration. */
    slamMs: z.number().int().min(100).max(5000),
    /** Tar-goo expansion/pour duration. */
    gooMs: z.number().int().min(100).max(5000),
    /** Pause to read the 5 axes before the PG enters. */
    axisReadMs: z.number().int().min(0).max(5000),
    /** Agency burst duration. */
    burstMs: z.number().int().min(100).max(5000),
    /** Risk pour duration. */
    pourMs: z.number().int().min(100).max(5000),
    /** Ball spin duration. */
    spinMs: z.number().int().min(500).max(20000),
    /** Snap/settle duration. */
    snapMs: z.number().int().min(0).max(5000),
  }),
...
/** Inferred V6.4 presentation config type. */
export type AstrolabeV64Config = z.infer<typeof astrolabeV64ConfigSchema>;

/**
 * Canonical V6.4 presentation parameters. Every value is a token — no
 * component or engine code may hardcode them.
 */
export const astrolabeV64Config: AstrolabeV64Config = astrolabeV64ConfigSchema.parse({
  phaseDurations: {
    ringMs: 140,
    slamMs: 900,
    gooMs: 1100,
