...
/**
 * Per-day deterministic production or consumption entry used by tick streaming.
 */
export interface ResourceRateDefinition {
  /** Resource id defined in config.resources */
  resourceId: string;
  /**
   * Amount generated/consumed per in-game day before multipliers.
   * Positive values add to inventory, negative values consume.
   */
  amountPerDay: number;
}
...
export interface ActivityDefinition {
  ...
  /**
   * Optional flag that enables per-tick payout for this job.
   */
  supportsPartialResolution?: boolean;
  /**
   * Declares that the job keeps residents assigned indefinitely until manually removed.
   */
  continuousJob?: boolean;
  /**
   * Enables auto-rescheduling whenever the activity completes.
   */
  supportsAutoRepeat?: boolean;
  /**
   * Total fatigue budget per in-game day consumed while this action runs.
   * Used to derive per-tick fatigue delta.
   */
  dailyFatigueCost?: number;
  /**
   * Deterministic per-day rewards streamed while the job runs.
   */
  dailyRewardProfile?: ResourceRateDefinition[];
  /**
   * Deterministic per-day costs (maintenance/upkeep) streamed while the job runs.
   */
  perTickCostProfile?: ResourceRateDefinition[];
  /**
   * Controls whether the activity pays out continuously ("tick") or only once at the end ("final").
   * Useful for quest-style encounters that should feel all-or-nothing.
   */
  resolutionMode?: ActivityResolutionMode;
  /**
   * When true, prevents residents assigned to this activity from being manually removed
   * until the run completes. Ideal for high-risk quests that must lock crews in place.
   */
  lockAssignmentsDuringRun?: boolean;
}

/** Declarative quest taxonomy definition used for analytics/UI. */
export interface QuestTypeMatcher {
  /** Keywords that must appear anywhere in the quest id (case-insensitive). */
  includes?: string[];
  /** Prefixes that, if matched, classify the quest. */
  prefixes?: string[];
  /** Optional regex pattern string applied case-insensitively. */
  regex?: string;
}
...
export interface QuestTypeDefinition {
  ...
  colorClass?: string;
  icon?: string;
  matchers?: QuestTypeMatcher[];
  /** Lower numbers take precedence when multiple matchers apply. */
  priority?: number;
  /** Marks a type as the fallback when no matcher hits. */
  isFallback?: boolean;
}
...
export interface QuestPhase {
  ...
  durationUnits: 'ticks' | 'hours' | 'days';
  requirements?: QuestPhaseRequirement;
  successEffects?: PhaseOutcomeEffects;
  failureEffects?: PhaseOutcomeEffects;
  copy: PhaseCopy;
  icon?: string;
  telemetryTags?: string[];
  riskProfile?: QuestPhaseRiskProfile;
}
...
export interface PhaseOutcomeEffects {
  resources?: ResourceDeltaDefinition[];
  reputation?: Record<string, number>;
  unlockActivityIds?: string[];
  notes?: string;
}

export interface QuestPhaseRiskProfile {
  injuryChance: number;
  deathChance: number;
  fatigueCost?: number;
  threatLabel?: string;
...
export interface QuestBlueprint {
  ...
  slotId?: string;
  tags?: string[];
  difficulty?: 'story' | 'skirmish' | 'dangerous' | 'heroic';
  narrative?: string;
  icon?: string;
  rewards?: QuestRewardsDefinition;
  telemetry: QuestBlueprintTelemetry;
  phases: QuestPhase[];
}
...
export interface QuestBlueprintTelemetry {
  eventId: string;
  enabled: boolean;
  tags?: string[];
}
...
/**
 * Configuration for production scaling with multiple workers and stat bonuses.
 * Used by ProductionEngine to calculate output from buildings and jobs.
 */
export interface ProductionScalingConfig {
  /**
   * Diminishing returns factor applied to each additional worker beyond the first.
   * Example: 0.8 means 2nd worker produces 80% of 1st, 3rd produces 64%, etc.
   * Range: 0.0-1.0, default: 0.8
   */
  diminishingReturnsFactor: number;
  /**
   * Multiplier per stat point for production bonus.
   * Example: 0.1 means strength 5 gives +50% production (1 + 0.1 * 5 = 1.5x)
   * Default: 0.1
   */
  statMultiplierPerPoint: number;
  /**
   * Whether to apply diminishing returns to the first worker.
   * If false, first worker always gets full output, diminishing starts from 2nd.
   * Default: false
   */
  applyDiminishingToFirstWorker?: boolean;
  /**
   * Optional cap on total production multiplier from stats.
   * Example: 3.0 means max 300% production regardless of stat values.
   */
  maxStatMultiplier?: number;
}

/**
 * Upgrade definition for buildings with level progression.
 */
export interface BuildingUpgrade {
  /**
   * Target level for this upgrade (must be > current level).
   */
  level: number;
  /**
   * Resource costs to perform this upgrade.
   * Keys are resource IDs, values are amounts required.
   */
  costs: Record<string, number>;
  /**
   * Optional cost formula for dynamic scaling.
   * If provided, overrides static costs.
   * Example: 'baseGold * 1.15^(level-1)'
   */
  costFormula?: string;
  /**
   * Optional production multiplier applied at this level.
   * Example: 1.5 means +50% output
   */
  productionMultiplier?: number;
  /**
   * Optional capacity increase (e.g., worker slots, storage).
   */
  capacityIncrease?: number;
  /**
   * Optional human-readable notes for designers.
   */
  notes?: string;
}
...
export interface GlobalRules {
  ...
  /**
   * Production scaling configuration for buildings and jobs.
   */
  ...
  /** Optional override for per-night-tick recovery (defaults to fatigueRecoveryPerDay / ticksPerNight). */
  fatigueRecoveryPerNightTick?: number;
  ...
  /**
   * Optional resident templates for initial roster seeding.
   * Used by gameplay bootstrap to populate the starting resident roster.
   */
