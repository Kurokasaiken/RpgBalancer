/**
 * Visual token contract for the POI Materic V5 marker.
 *
...
const tokensSchema = z.object({
  ...
  rim: z.object({
    ...
    /** Width of the raised ring band that carries the engraved icons. */
    bandWidth: z.number(),
    /** Opacity of the directional chiaroscuro glaze over the ring band. */
    chiaroscuroOpacity: z.number(),
    /** Shadow shift for the carved icon, in scaled letter units. */
    shadowShift: z.tuple([z.number(), z.number()]),
    /** Light shift for the carved icon, in scaled letter units. */
    lightShift: z.tuple([z.number(), z.number()]),
  }),
  ...
  surface: z.object({
    ...
    /** Opacity of the fractal-noise overlay on metal surfaces. */
...
export type PoiMatericV5Tokens = z.infer<typeof tokensSchema>;

export const POI_MATERIC_V5_TOKENS: PoiMatericV5Tokens = tokensSchema.parse({
  metal: {
    light: '#fce890',
    mid: '#c09030',
    dark: '#200e02',
  ...
  rim: {
    ...
    strokeWidth: 1.3,
    bandWidth: 8,
    chiaroscuroOpacity: 0.72,
    shadowShift: [0.7, 0.85],
    lightShift: [-0.45, -0.55],
  },
