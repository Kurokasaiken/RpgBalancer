...
export const ResourceRateDefinitionSchema = z.object({
  resourceId: z.string().min(1),
...
export const ActivityDefinitionSchema = z
  .object({
    ...
    supportsPartialResolution: z.boolean().optional(),
    continuousJob: z.boolean().optional(),
    supportsAutoRepeat: z.boolean().optional(),
    dailyFatigueCost: z.number().optional(),
    dailyRewardProfile: z.array(ResourceRateDefinitionSchema).optional(),
    perTickCostProfile: z.array(ResourceRateDefinitionSchema).optional(),
  });
...
const QuestPhaseSchema = z.object({
  id: z.string().min(1),
  type: z.enum(['check', 'fight', 'stealth', 'trap', 'explore']),
  label: z.string().min(1),
  icon: z.string().optional(),
  description: z.string().optional(),
  narrative: z.string().optional(),
  requirements: QuestPhaseRequirementSchema.optional(),
  heroicMetadata: z.object({
    hasDeathRisk: z.boolean().optional(),
    deathRiskThreshold: z.number().min(0).max(100).optional(),
  }).optional(),
});

const QuestTypeMatcherSchema = z.object({
  includes: z.array(z.string()).optional(),
  prefixes: z.array(z.string()).optional(),
  regex: z.string().optional(),
});

export const QuestTypeDefinitionSchema = z.object({
  id: z.string().min(1),
  label: z.string().min(1),
  description: z.string().optional(),
  colorClass: z.string().optional(),
  icon: z.string().optional(),
  matchers: z.array(QuestTypeMatcherSchema).optional(),
...
export const GlobalRulesSchema = z.object({
  ...
  fatigueRecoveryPerNightTick: z.number().optional(),
  dayLengthInTimeUnits: z.number(),
  ticksPerDay: z.number().int().min(1).optional(),
  dayNightCycle: z
    .object({
      dayTimeUnits: z.number().int().min(1),
      nightTimeUnits: z.number().int().min(1),
    })
    .optional(),
  ticksPerNight: z.number().int().min(1).optional(),
  productionHaltFatigueThreshold: z.number().min(0).max(1).optional(),
...
export const IdleVillageConfigSchema = z.object({
  ...
  questTypes: z.record(z.string(), QuestTypeDefinitionSchema),
