...
const createNormalizedDefaults = (): IdleVillageConfig =>
  applyIdleVillageConfigDefaults(clone(DEFAULT_IDLE_VILLAGE_CONFIG));
...
export const useIdleVillageConfigStore = create<IdleVillageConfigStoreState>((set, get) => ({
  config: createNormalizedDefaults(),
  history: [],
  initialized: false,
  ...
  async initializeConfig() {
    ...
    try {
      ...
      const fallback = createNormalizedDefaults();
      set({
        config: fallback,
        history: [],
        initialized: true,
  ...
  importConfig(json, description = 'Imported IdleVillageConfig') {
    try {
      ...
      const normalized = applyIdleVillageConfigDefaults(parsed);
      return get().saveConfig(normalized, description);
    } catch (error) {
      return { success: false, error: (error as Error).message };
    }
  },
   ...
   */
  resetConfig() {
    const defaults = createNormalizedDefaults();
