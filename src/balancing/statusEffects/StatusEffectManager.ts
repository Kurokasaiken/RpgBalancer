...
export interface ShieldEffect extends StatusEffect {
    ...
    currentShield?: number; // Optional runtime tracker (falls back to shieldAmount)
}
                ...
                case 'debuff': {
                    ...
                    Object.entries(modEffect.statChanges).forEach(([stat, value]) => {
                        if (value !== undefined && typeof value === 'number') {
                            ...
                            (result.modifiedStats as unknown as Record<string, number>)[key] =
                ...
                Object.entries(modEffect.statChanges).forEach(([stat, value]) => {
                    if (typeof value === 'number') {
                        ...
                        if (typeof current === 'number') {
                            ...
                            numericStats[key] = current + value;
                        }
                    }
                });
            }
...
export const StatusEffectFactory = {
    ...
    /**
     * Create a Shield effect (temporary HP buffer)
     *
     * Ensures shieldAmount and currentShield stay in sync.
     *
     * @param name - Display name of the shield effect
     * @param shieldAmount - Total HP absorbed by the shield
     * @param duration - Number of turns before the shield expires
     * @param source - What applied this shield (spell/ability)
     * @returns Shield effect definition
     */
    createShield(
        name: string,
        shieldAmount: number,
        ...
        const normalizedAmount = Math.max(0, shieldAmount);
        return {
            id: `shield_${++this._idCounter}`,
            name,
            type: 'shield',
            duration,
            stackable: false,
            shieldAmount: normalizedAmount,
            currentShield: normalizedAmount,
            source
        };
    },
