...
export function calculateSpellBudget(
    ...
    for (const field of balanceableFields) {
        const spellValue = (spell as unknown as Record<string, unknown>)[field] || 0;
        const baselineValue = (baseline as unknown as Record<string, unknown>)[field] || 0;
        ...
        const delta = (spellValue as number) - (baselineValue as number);
        budget += delta * weight;
    }
