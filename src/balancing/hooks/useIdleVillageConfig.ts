...
type IdleVillageConfigStoreSnapshot = ReturnType<typeof useIdleVillageConfigStore.getState>;
...
export function useIdleVillageConfig(): UseIdleVillageConfigReturn {
  ...
  useEffect(() => {
    ...
    const unsubscribe = useIdleVillageConfigStore.subscribe((state, prevState) => {
      const changedKeys = Object.keys(state).filter((key) => {
        const typedKey = key as keyof IdleVillageConfigStoreSnapshot;
        return state[typedKey] !== prevState[typedKey];
      });

      if (changedKeys.length > 0) {
        console.debug('[IdleVillageConfigTrace] storeUpdate', { changedKeys });
      }
    });
