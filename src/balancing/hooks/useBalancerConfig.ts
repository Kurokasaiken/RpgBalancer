...
import { BalancerHistoryStore, defaultHistoryStore } from '../config/BalancerHistoryStore';
...
export function useBalancerConfig(): UseBalancerConfigReturn {
  ...
  const historyStoreRef = useRef<BalancerHistoryStore | null>(null);
  ...
  useEffect(() => {
    if (!historyStoreRef.current) {
      historyStoreRef.current = defaultHistoryStore;
      historyStoreRef.current.initialize().then(() => {
        const state = historyStoreRef.current!.getState();
        setHistory(historyStoreRef.current!.getHistory());
  ...
  const saveConfig = useCallback(
    async (next: BalancerConfig, description: string) => {
      // Save to history store first
      if (historyStoreRef.current) {
        await historyStoreRef.current.pushSnapshot(next, description);
        const state = historyStoreRef.current.getState();
        setHistory(historyStoreRef.current.getHistory());
        setCanUndo(state.canUndo);
        setCanRedo(state.canRedo);
  ...
  // === HISTORY ===
  /**
   * History queue operation tracking for race condition prevention.
   */
  const historyOperationRef = useRef<string | null>(null);
  const historyQueueRef = useRef<Array<() => Promise<void>>>([]);
  const isProcessingQueueRef = useRef(false);

  /**
   * Processes the history queue sequentially to prevent race conditions.
   * Ensures that undo/redo operations are executed in order.
   */
  const processHistoryQueue = useCallback(async (): Promise<void> => {
    if (isProcessingQueueRef.current || historyQueueRef.current.length === 0) {
      return;
    }
    ...
    try {
      while (historyQueueRef.current.length > 0) {
        const operation = historyQueueRef.current.shift();
        if (operation) {
          await operation();
        }
      }
  ...
  /**
   * Queues a history operation with a unique identifier to prevent duplicates.
   * 
   * @param operationId - Unique identifier for the operation
   * @param operation - The async operation to queue
   */
  const queueHistoryOperation = useCallback(
    async (operationId: string, operation: () => Promise<void>): Promise<void> => {
      // Prevent duplicate operations
      if (historyOperationRef.current === operationId) {
        return;
      }

      historyOperationRef.current = operationId;
      historyQueueRef.current.push(operation);
      await processHistoryQueue();
      
      // Clear operation ID after processing
      setTimeout(() => {
  ...
  const undo = useCallback(async () => {
    const operationId = `undo_${Date.now()}_${Math.random().toString(36).slice(2)}`;
    
    await queueHistoryOperation(operationId, async () => {
      if (historyStoreRef.current) {
        const previousConfig = await historyStoreRef.current.undo();
        if (previousConfig) {
  ...
  const redo = useCallback(async () => {
    const operationId = `redo_${Date.now()}_${Math.random().toString(36).slice(2)}`;
    
    await queueHistoryOperation(operationId, async () => {
      if (historyStoreRef.current) {
        const nextConfig = await historyStoreRef.current.redo();
        if (nextConfig) {
