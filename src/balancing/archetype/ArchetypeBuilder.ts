...
export const ArchetypeBuilder = {
    ...
    calculateStatValues: (
        ...
        (Object.keys(allocation) as (keyof StatAllocation)[]).forEach(statName => {
            ...
            if (allocPercent === 0) {
                (statBlock as Record<string, number>)[statName] = 0;
                return;
            }
            ...
            if (!weight) {
                ...
                (statBlock as Record<string, number>)[statName] = hpEq; // Fallback: 1:1 conversion
                return;
            }
            ...
            (statBlock as Record<string, number>)[statName] = Math.round(statValue * 100) / 100; // Round to 2 decimals
        });
