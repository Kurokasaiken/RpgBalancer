...
```bash
# Run all tests (CI-safe)
npm run test -- src/balancing/__tests__/CombatTestFramework.test.ts

# Run with verbose output
npm run test -- src/balancing/__tests__/CombatTestFramework.test.ts --reporter=verbose

# Watch mode (local development only)
npm run test:watch -- --filter=src/balancing/__tests__/CombatTestFramework.test.ts
```

---

## Phase 10.5 - Stress Test Archetype Generator

### Overview

The **StressTestArchetypeGenerator** is a deterministic system for generating stress test archetypes used in Phase 10.5 stat efficiency analysis. It creates single-stat and pair-stat archetypes based on BalancerConfig weights.

### Key Concepts

- **pairScore** = win rate della build simulata (archetipo "colonna") sul totale dei duelli vs baseline (`wins_col / totalSimulations`).
- **expectedScore** = media aritmetica dei win rate dei due archetipi single stat coinvolti.
- **synergyMultiplier** = `pairScore / expectedScore`.  
  - `> 1.15` ⇒ `isOpSynergy = true`  
  - `< 0.95` ⇒ `isWeakSynergy = true`

### Usage

```typescript
import { StressTestArchetypeGenerator } from '../stressTesting/StressTestArchetypeGenerator';

// Create generator (async constructor loads config)
...
The generator respects configuration from `archetypeSeeds.ts`:
- **excludeDerived**: Skip formula-derived stats
- **minWeight**: Filter low-weight stats
- **excludeIncompatiblePairs**: Skip incompatible stat combinations
- **useSynergyMultipliers**: Use synergy thresholds for pair generation
- **maxPairs**: Limit combinatorial explosion

### Deterministic Seeding

All generation is deterministic using the TestRNG system:
- Same seed → identical archetypes
- Different seeds → different combinations
- Default seed: 12345 (from config)

### Export Formats

```typescript
// Export to JSON (for analysis)
const json = generator.exportArchetypeMatrix('json');

// Export to CSV (for Excel)
const csv = generator.exportArchetypeMatrix('csv');

// Export to Markdown (for documentation)
const md = generator.exportArchetypeMatrix('markdown');
```

### Integration with Marginal Utility Calculator

The generated archetypes feed into the Marginal Utility Calculator for Phase 10.5 efficiency analysis:
1. Generate archetypes with StressTestArchetypeGenerator
2. Run round-robin simulations
3. Calculate pairScore/expectedScore/synergyMultiplier
4. Feed results to UI components (SynergyHeatmap, StatProfileRadar)

**KS-105 Status**: ✅ **COMPLETED 2026-01-13** - StressTestArchetypeGenerator fully implemented with config-first design, deterministic seeding, and comprehensive testing

---

### Stress Testing Utility Reference
...
Questa metrica è condivisa da `MarginalUtilityCalculator`, dai test Vitest in `src/balancing/stressTesting/__tests__` e dalle esportazioni CSV generate dagli stessi moduli. Evitare calcoli locali nei componenti UI: leggere sempre i valori dall’analisi prodotta.
