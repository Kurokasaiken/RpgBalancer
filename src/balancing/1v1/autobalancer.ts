...
function proposeNerfs(
    ...
    for (let i = 0; i < Math.min(numStatsToNerf, sortedStats.length); i++) {
        ...
        const currentValue = (archetype.stats as unknown as Record<string, number>)[statKey];
...
export function applyAdjustments(
    ...
    for (const adj of adjustments) {
        ...
        // Apply adjustment
        (archetype.stats as unknown as Record<string, number>)[adj.statKey] = adj.proposedValue;
    }
