...
import type { ArchetypeTemplate } from '../archetype/types';
...
export async function loadTestArchetypes(): Promise<Archetype[]> {
    ...
    for (const template of testArchetypesJSON) {
        const instance = ArchetypeBuilder.buildArchetype(template as unknown as ArchetypeTemplate, budget);

        archetypes.push({
            id: template.id,
            name: template.name,
            role: template.category as string,
