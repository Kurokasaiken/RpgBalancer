...
export class CombatSimulator {
    ...
    simulate(config: CombatConfig): CombatResult {
        ...
        const turnByTurnLog = enableDetailedLogging ? undefined : undefined;
     ...
     */
    private entityToStatBlock(entity: unknown): StatBlock {
        ...
        return {
            ...
            hp: e.hp ?? BASELINE_STATS.hp,
            damage: e.attack ?? e.damage ?? BASELINE_STATS.damage,
            armor: e.defense ?? e.armor ?? BASELINE_STATS.armor,
            resistance: e.resistance ?? BASELINE_STATS.resistance,

            // Hit chance stats
            txc: e.txc ?? BASELINE_STATS.txc,
            evasion: e.evasion ?? BASELINE_STATS.evasion,

            // Critical stats
            critChance: e.critChance ?? BASELINE_STATS.critChance,
            critMult: e.critMult ?? BASELINE_STATS.critMult,
            critTxCBonus: e.critTxCBonus ?? BASELINE_STATS.critTxCBonus,

            // Fail stats
            failChance: e.failChance ?? BASELINE_STATS.failChance,
            failMult: e.failMult ?? BASELINE_STATS.failMult,
            failTxCMalus: e.failTxCMalus ?? BASELINE_STATS.failTxCMalus,

            // Mitigation stats
            armorPen: e.armorPen ?? BASELINE_STATS.armorPen,
            penPercent: e.penPercent ?? BASELINE_STATS.penPercent,

            // Sustain stats
            lifesteal: e.lifesteal ?? BASELINE_STATS.lifesteal,
            regen: e.regen ?? BASELINE_STATS.regen,
            ward: e.ward ?? BASELINE_STATS.ward,
            block: e.block ?? BASELINE_STATS.block,
            energyShield: e.energyShield ?? BASELINE_STATS.energyShield,
            thorns: e.thorns ?? BASELINE_STATS.thorns,

            // Timing stats
            cooldownReduction: e.cooldownReduction ?? BASELINE_STATS.cooldownReduction,
            castSpeed: e.castSpeed ?? BASELINE_STATS.castSpeed,
            movementSpeed: e.movementSpeed ?? BASELINE_STATS.movementSpeed,

            // Config flags
            configFlatFirst: e.configFlatFirst ?? BASELINE_STATS.configFlatFirst,
            configApplyBeforeCrit: e.configApplyBeforeCrit ?? BASELINE_STATS.configApplyBeforeCrit,
        } as StatBlock;
    }
