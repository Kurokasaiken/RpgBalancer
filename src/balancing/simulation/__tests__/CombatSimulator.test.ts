...
/**
 * Builds an EntityStats object from a partial stat block, ensuring required combat fields exist.
 */
function toEntityStats(stats: Partial<typeof BASELINE_STATS> & { name: string }): EntityStats {
    return {
        ...stats,
        hp: stats.hp ?? BASELINE_STATS.hp,
        attack: stats.damage ?? BASELINE_STATS.damage,
        defense: stats.armor ?? BASELINE_STATS.armor,
    };
}
