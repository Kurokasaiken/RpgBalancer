...
type NumericStatKey = {
  [K in keyof StatBlock]: StatBlock[K] extends number ? K : never;
}[keyof StatBlock];

/**
 * Checks whether the provided StatBlock key holds numeric values.
 */
function isNumericStatKey(key: keyof StatBlock): key is NumericStatKey {
  return key !== 'configFlatFirst' && key !== 'configApplyBeforeCrit';
}

/**
 * Assigns a numeric value to a StatBlock key in a type-safe manner.
 */
function setNumericStatValue(block: StatBlock, key: NumericStatKey, value: number): void {
  block[key] = value;
}

/**
 * Applies an additive delta to a numeric StatBlock key.
 */
function incrementNumericStatValue(block: StatBlock, key: NumericStatKey, delta: number): void {
  block[key] = block[key] + delta;
}
...
function buildBaselineFromConfig(config: BalancerConfig): StatBlock {
  ...
  Object.values(config.stats).forEach((stat) => {
    ...
    if (!(key in base) || !isNumericStatKey(key)) return;
    setNumericStatValue(base, key, stat.defaultValue);
  });
...
export async function estimateStatPointCosts(
  ...
  for (const def of Object.values(config.stats)) {
    ...
    if (!(key in baseline) || !isNumericStatKey(key)) continue;

    const delta = getDeltaForStat(def);
    if (delta <= 0) continue;

    const boosted = cloneStatBlock(baseline);
    incrementNumericStatValue(boosted, key, delta);
