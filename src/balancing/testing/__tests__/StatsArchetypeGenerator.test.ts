...
describe('StatsArchetypeGenerator', () => {
  ...
  const buildGeneratorBaseline = (config = DEFAULT_CONFIG): StatBlock => {
    ...
    Object.values(config.stats).forEach((stat) => {
      ...
      if (key in baseline) {
        (baseline as unknown as Record<string, number>)[key] = stat.defaultValue;
      }
    });
    return baseline;
  };
  ...
  test('single-stat archetype applies correct delta based on getStatWeight', () => {
    ...
    for (const statId of candidates) {
      ...
      const baselineValue = (generatorBaseline as unknown as Record<string, number>)[statId] ?? 0;
      ...
      const actualValue = (archetype.stats as unknown as Record<string, number>)[statId];
