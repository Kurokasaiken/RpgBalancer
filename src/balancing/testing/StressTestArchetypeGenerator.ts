...
export type Archetype = StatsArchetype;
...
function buildBaseFromConfig(config: BalancerConfig): StatBlock {
  ...
  Object.values(config.stats).forEach((stat) => {
    ...
    if (key in base) {
      (base as unknown as Record<string, number>)[key] = stat.defaultValue;
    }
  });
...
export class StatsArchetypeGenerator {
  ...
  private buildScaledBaseline(pointsPerStat: number): StatBlock {
    ...
    for (const statId of this.nonDerivedStatIds) {
      ...
      const current = (stats as unknown as Record<string, number>)[statId] ?? 0;
      ...
      (stats as unknown as Record<string, number>)[statId] = current + delta;
    }
    ...
    for (const pointsPerStat of pointTiers) {
      ...
      for (let i = 0; i < ids.length; i++) {
        for (let j = i + 1; j < ids.length; j++) {
          ...
          const currentA = (stats as unknown as Record<string, number>)[a] ?? 0;
          const currentB = (stats as unknown as Record<string, number>)[b] ?? 0;
          ...
          (stats as unknown as Record<string, number>)[a] = currentA + extraDeltaA;
          (stats as unknown as Record<string, number>)[b] = currentB + extraDeltaB;

          result.push({
            id: `stress-pair-${a}-${b}-${pointsPerStat}`,
            name: `Pair +${pointsPerStat} ${defA.label} & +${pointsPerStat} ${defB.label}`,
            type: 'pair-stat',
            seed: 2000 + pointsPerStat + i * 100 + j,
            stats,
            testedStats: [a, b],
            pointsPerStat,
