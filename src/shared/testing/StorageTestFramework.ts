      ...
      const promises = Array(5)
        ...
        .map((_, i) => {
          ...
          if (typeof modifiedData === 'object' && modifiedData !== null) {
            (modifiedData as { _concurrent_id?: unknown })._concurrent_id = i;
          }
  ...
  async testTypePreservation(data: T): Promise<TestResult> {
    return this.runTest('Type Preservation', async () => {
      ...
      if (originalType === 'object') {
        const originalKeys = Object.keys(data as Record<string, unknown>).sort();
        const loadedKeys = Object.keys(loaded as Record<string, unknown>).sort();
...
// Helper exports for Storage Audit Tool
export { StorageTestFramework as default };
