...
> **LEGACY NOTICE (2025-12-30)**  
> Questo piano “Map Only” è stato superato dal focus sulla **Village Sandbox** e dalle roadmap Phase 12 / Atomic Sandbox. Manteniamo il documento solo a fini storici (vision, storytelling, riferimenti Trial of Fire). Le nuove decisioni operative vanno aggiornate in:
>
> - `docs/plans/village_sandbox_refactor_plan.md`
> - `docs/plans/idle_village_atomic_sandbox_plan.md`
> - `docs/plans/idle_village_plan.md`
>
> Qualsiasi divergenza tra questo file e i piani sopra elencati va risolta seguendo la documentazione Sandbox.
...
## 7. Terrain Modifier Config Tool (NP-092)

### 7.1 Obiettivo

Fornire ai designer un editor config-first per i modifier di terreno (bonus/malus sui map slot) con preview stratificata e persistence asincrona. Il tool vive sotto `src/ui/idleVillage/tools/TerrainModifierTool.tsx` e usa il nuovo hook `useTerrainModifiers`.

### 7.2 Schema & Persistence
- Tutti i modifier sono definiti tramite `TerrainModifierDefinition` in `src/ui/idleVillage/config/terrainModifierConfig.ts`.
- Le proprietà obbligatorie: `id`, `label`, `slotIds`, `effectType` (`production`, `risk`, `safety`, `mobility`), `magnitude` (±100%), `color`, `intensity`, `layerId`, `isEnabled`.
- Layer metadata (`TerrainModifierLayerConfig`) determina stacking order e default visibility. I layer di default sono Environment / Structures / Events.
- La persistence (`saveData/loadData`) usa chiave `idleVillage-terrain-modifiers`, con schema zod (`TerrainModifierPersistenceSchema`) e versioning `TERRAIN_MODIFIER_PERSISTENCE_VERSION`.


### 7.3 UI & Workflow
1. **Sidebar** – lista modifier + pannello layer:
   - CTA `+ Add` crea un template basato sul primo slot disponibile.
   - Toggle layer (`Hide/Show`) e reorder (`↑/↓`) agiscono sullo state.
2. **Layered Preview** – card compatte sovrapposte alla mappa (posizioni calcolate da `computeSlotPercentPosition`):
   - Mostra aggregati per slot: effetto percentuale, layer abilitati, intensità.
   - Solo i layer `visible` vengono renderizzati (lo stacking segue `order`).
3. **Inspector** – editing proprietà:
   - Slider per `magnitude` / `intensity`, select per `effectType` e `pattern`, picker dei slot target.
   - Azioni rapide: Enable/Disable, Duplicate, Delete.

### 7.4 Telemetria & Safeguard

- Il hook emette `terrain_modifier_saved` via sandbox diagnostics ogni volta che un salvataggio va a buon fine (incluso `modifierCount` e `layerCount`).
- Operazioni vietate: il tool scrive solo nella preview; nessun binding diretto a `locationSlots` runtime.
- Guardrail obbligatori:
  1. `npm run lint -- src/ui/idleVillage`
  2. `npm run test -- tests/unit/idleVillage/TerrainModifierTool.test.tsx`
  3. `npm run build:check`
  4. `npm run kanban:lint`

### 7.5 Layering Stack (configurabile)
| Layer ID    | Uso                               | Colore hint | Note                                  |
|-------------|-----------------------------------|-------------|---------------------------------------|
| environment | Biomi statici (foreste, paludi)   | #4c956c     | Pattern default: diagonal             |
| structures  | Costruzioni/upgrades giocatore    | #c9a227     | Pattern default: dots                 |
| events      | Modificatori temporanei/quest     | #e76f51     | Pattern default: solid (overlay rosso) |

Aggiornare questa tabella quando si introducono nuovi layer o pattern, mantenendo la source of truth dentro `terrainModifierConfig.ts`.
