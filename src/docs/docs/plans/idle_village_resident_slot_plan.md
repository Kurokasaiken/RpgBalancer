...
 
- Created `useResidentSlotController` hook with infinite slot support (virtualSlots = assignedCount + 1) and shared telemetry helpers (`createResidentSlotTelemetryPayload`).
- Shared contract now documented in `src/ui/idleVillage/slots/types.ts` (view models, controller API, warnings, bloom states).
- Tests updated in `src/ui/idleVillage/slots/useResidentSlotController.test.ts` to cover duplicate placeholders, bloom aggregation, slot fullness, and telemetry payloads (8/8 Vitest).
...
### Phase F – Testing & Documentation
1. **Vitest:**
   - `tests/unit/idleVillage/ResidentSlotController.test.ts`: infinite growth, `assign`/`clear`, stat validation errors → dropState `invalid`.
   - `tests/unit/idleVillage/ResidentSlotRack.test.tsx`: overflow snapshot, bloom toggle, hover/focus accessibility.
   - `tests/unit/idleVillage/ActivitySlot.test.tsx`: halo/timer props, highlight removal after `onDrop`/`onCancel`.
2. **Integration:** `tests/integration/idleVillage/ResidentSlotParity.test.tsx` covering Theater + VerbDetailCard parity (drag success, reject, placeholder spawn, telemetry hook).
3. **Playwright:** `tests/e2e/idleVillage/residentSlots.drag.spec.ts` – drag into Theater slot, open VerbDetailCard, drop resident, verify placeholder creation and scroll hints when slots > 6, ensure blocked drop shows tooltip and worker reverts.
4. **Docs:** update `idle_village_plan.md` + `idle_village_tasks.md` + `MASTER_PLAN.md` referencing this plan.
