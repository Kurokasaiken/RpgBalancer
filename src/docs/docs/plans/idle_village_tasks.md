...
- [ ] Implement tick-based fatigue/reward streaming (per-Day budget sliced per tick)
- [ ] Add night-phase fatigue recovery loop tied to `fatigueRecoveryPerDay`
- [ ] Support continuous job assignments that rest at night and resume production automatically
- [ ] Ensure Jobs flagged as partial grant proportional rewards/resources every tick
- [ ] Expose `supportsPartialResolution`, `continuousJob`, `supportsAutoRepeat` toggles + tick budgets inside the Activities authoring UI and persist via config/PersistenceService
