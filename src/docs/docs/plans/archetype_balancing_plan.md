...
#### 2.0 Helper Allocation APIs
- **createBalancedAllocation()** (in `src/balancing/archetype/constants.ts` re-exported from `allocationDefaults`)
  - Usa `withAllocationDefaults` per assicurare che ogni stat sia presente.
  - Restituisce un profilo equilibrato (≈100% budget) che può essere usato come baseline per:
    1. Seed rapido di nuove varianti nel builder UI.
    2. Opponenti standard nei test automatizzati (es. `ArchetypeBalanceCard`).
    3. Previews/placeholder quando un archetipo non ha ancora un allocation definita.
  - **Come usarlo:** `import { createBalancedAllocation } from '@/balancing/archetype/constants';`
    quindi assegnare il risultato direttamente al campo `allocation` della template oppure usarlo come base da modificare.
  - Mantiene coerenza con la filosofia config-first: modificare i valori solo dentro `allocationDefaults.ts` se serve un nuovo profilo globale.
