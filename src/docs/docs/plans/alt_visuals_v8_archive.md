# Alt Visuals v8 Archive – "Obsidian Meridian"

> **Status:** Dropped on 2026-01-04 – rollback requested by WS6 leadership after repeated regressions (frame pacing, PIXI memory spikes, inconsistent stat randomization) and stakeholder preference to keep the SkillCheck Preview lab aligned with the canonical V6 Asterism view. This archive preserves the historical plan for reference.

## Rollback Summary

- **Reason:** V8 PIXI field introduced rendering instability (mobile tearing, stuck timeline) and distracted from the mandatory SkillCheck UX goals. QA reported 4 blocking regressions on Pixel 5/6 plus two deterministic crashes in the dice controller. Stakeholders requested to freeze on V6-only visuals while the alt visuals backlog is re-evaluated.
- **Impact:** SkillCheck Preview lab reverts to the single V6 Asterism tab. No additional PIXI assets or filters will ship in PC-M2. The V8 asset manifest and helper modules remain in git history but are not referenced by the UI.
- **Next Steps:** Use this archive if/when V8 is revived. Any future revival must run through a fresh WS scope with perf guardrails and config-first toggles.
...
- [x] Assets downloaded under `public/assets/alt-visuals/v8/` with manifest + licensing info.
- [x] `altVisualsV8Theme.ts` + types exported.
- [x] `AltVisualsV8ObsidianField.tsx` implemented.
- [x] `SkillCheckPreviewPage` updated (tab list, default selection, storage key migration).
- [ ] `AltVisualsV7PixiField.tsx` removed.
- [x] Tests updated (lint, Playwright scenario for v8).
- [x] Documentation/CHANGELOG entry explaining v8 rollout and rationale.
