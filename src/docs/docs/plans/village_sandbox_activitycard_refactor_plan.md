...
1. **Unify Activity Rendering**  
   - Replace ad-hoc map markers and legacy VerbCard props with the new ActivityCard detail data pipeline (`icon`, `progressFraction`, `elapsedSeconds`, `totalDuration`, `injuryPercentage`, `deathPercentage`, `assignedCount`, `totalSlots`, `visualVariant`, `progressStyle`) while keeping **map-facing mini-cards halo-only (no text, badges, or inline helper copy)**.
...
2. **Map Experience**  
   ...
   - Job mini-cards act purely as halo previews, but remain interactive: clicking (or hovering with a dragged resident for ≥600 ms) must launch the full `DetailPanelCard`, allowing assignments, drops, and Start/Pause controls exactly like the Village Sandbox.
...
4. **Quest/Job Offers**  
   - Offers displayed on map slots must share tone colors and call-to-action (“Accept”) buttons, backed by existing metadata (`activityToneId`, `risk` fields). Risk badges stay in HUD/detail views only, never on the halo mini-cards.
...
3. **Compact HUD Rows**  
   - Each row: `icon | label | reward | timer chip | action when applicable`. Risk stays in text form or tooltips within the HUD, not on halo cards.
