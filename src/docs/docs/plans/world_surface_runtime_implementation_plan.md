...
| 2026-07-23 | Cascade                 | HD layer proportion/position correction: `renderer.imageFit` set to `none`, per-layer `offsetX`/`offsetY`/`scale` added to manifest, `WorldSurfaceRendererConfigSchema` and `WorldSurfaceRenderer` updated to support `imageFit: 'none'` with `objectPosition: 0 0`. Safeguards pass. |
| 2026-07-23 | Cascade                 | Sea/Mare layer restored: `map finale no frame no nuvole.png` copied to `public/assets/world/wanderlust/base/layers/Mare.png`, `sea` layer added to manifest (zIndex 10, parallax x 0.02, tags water), manifest version bumped to 1.1.2. Safeguards pass. |
