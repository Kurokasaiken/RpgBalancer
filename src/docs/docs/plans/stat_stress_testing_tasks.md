...
- [x] Create `src/balancing/testing/StressTestArchetypeGenerator.ts`
- [x] Implement `generateSingleStatArchetipi()` method
  - [x] Read stats from BalancerConfig (no hardcoding)
  - [x] Skip derived stats (formulas)
  - [x] Calculate point allocation: weight * 25
  - [x] Generate StatBlock for each stat
  - [x] Return array of StressTestArchetype
- [x] Implement `generatePairStatArchetipi()` method
  - [x] Generate all C(n,2) combinations
  - [x] For each pair: baseline + (weightA * 25) + (weightB * 25)
  - [x] Return array of pair archetipi
- [x] Add validation:
  - [x] Ensure all stats present in generated archetipi
  - [x] Validate point allocations match weights
  - [x] Check for missing required stat fields
