...
2. **QuestChronicle Component** – Style Laboratory-styled card with icon stepper, phase status badges, and Theater 21:9 linkage.  
...
- `QuestBlueprint` now has a dedicated config module (`src/balancing/config/idleVillage/quests/questBlueprints.ts`) validated via `QuestBlueprintsSchema`. Each blueprint references `activityId`, `slotId`, telemetry metadata, difficulty, rewards, and an ordered list of `QuestPhase` nodes with duration/unit, requirements, effects, risk profile, and UI copy.

| Field | Type | Notes |
| --- | --- | --- |
| `id` | `string` | Unique blueprint identifier referenced by activities/TimeEngine |
| `name` | `string` | Display label surfaced in Quest Chronicle |
| `activityId` | `string` | Activity this blueprint augments |
| `slotId` | `string?` | Preferred map slot anchor |
| `tags` | `string[]` | Filtering/diagnostics |
| `difficulty` | `'story' &#124; 'skirmish' &#124; 'dangerous' &#124; 'heroic'` | Aligns with quest risk badges |
| `rewards` | `{ resources?: any; items?: any; unlockActivityIds?: any; reputation?: any }` | Config-first reward payload |
| `telemetry` | `{ eventId: string; enabled: boolean; tags?: string[] }` | Quest Blueprint telemetry config |
| `phases` | `QuestPhase[]` | Ordered phase nodes (see below) |

`QuestPhase` fields:

| Field | Type | Notes |
| --- | --- | --- |
| `id` | `string` | Phase node identifier |
| `title` | `string` | Phase label (UI + telemetry) |
| `type` | `QuestPhaseType` | Engine module: `check`, `fight`, `trap`, etc. |
| `durationValue` | `number` | Positive duration magnitude |
| `durationUnits` | `'ticks' &#124; 'hours' &#124; 'days'` | Normalized duration units |
| `requirements` | `{ statRequirement?, encounterId?, skillCheckId?, materials?, custom? }` | Module payload |
| `successEffects` / `failureEffects` | `{ resources?, reputation?, unlockActivityIds?, notes? }` | Hooks to TimeEngine/job resolver |
| `copy` | `{ summary, narrative, callToAction? }` | Narrative strings rendered in Chronicle/Theater |
| `riskProfile` | `{ injuryChance, deathChance, fatigueCost?, threatLabel? }` | Feeds Quest Risk Display |
| `telemetryTags` | `string[]?` | Additional trace tags |

Sample blueprint snippet (excerpt from `quest_city_rats`):
...
```json
{
  "quest_city_rats": {
    ...
    "telemetry": { "eventId": "quest_blueprint_city_rats", "enabled": true },
    "phases": [
      {
        "id": "scout_tunnels",
        ...
        "durationValue": 2,
        "durationUnits": "hours",
        "requirements": { "statRequirement": { "allOf": ["lantern"] } },
        "riskProfile": { "injuryChance": 5, "deathChance": 0 },
        "copy": {
          "summary": "Lanterniere e ausiliari marcano i tunnel infestati.",
          "narrative": "Gli scout mappano i condotti laterali.",
          "callToAction": "Schiera i lantern scout"
        }
      }
    ]
  }
...
- **2026-01-01:** Implemented the config-first `useQuestChronicle` hook plus fallback logic, wired it into `MapPage.tsx`, and landed dedicated Vitest coverage (`src/ui/idleVillage/hooks/__tests__/useQuestChronicle.test.ts`). Phase C is partially delivered (Chronicle view-model + modal integration); pending theater CTA + sandbox wiring.
...
1. Build `QuestChronicle.tsx` in `src/ui/idleVillage/components/`. ✅ (existing component)  
...
3. Add CTA header linking to Theater (prop callback). ⏳ (hook exists, CTA wiring pending Theater refactor)  
4. Integrate into `MapSlotVerbCluster` or overlay above quest slots. ✅ (`MapPage.tsx` modal uses new hook)  
5. Add story/narrative copy placeholders referencing config text. ✅ (hook pulls summary/narrative from blueprint metadata)
