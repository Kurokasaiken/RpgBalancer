...
} from '@/balancing/config/testData/balancerStorageDataset';
import { testBalancerConfigStore } from '@/shared/testing/StorageTestExamples';

const primary = buildPrimaryBalancerStorageDataset();
const alternate = buildAlternateBalancerStorageDataset();

const results = await testBalancerConfigStore(primary, alternate);
```

### Integration Recipe: BalancerConfigStore

Use this checklist whenever you need to validate the config-driven balancer persistence layer:
...
2. **Invoke the helper**
   ...
   const suite = await testBalancerConfigStore(
     buildPrimaryBalancerStorageDataset(),
     buildAlternateBalancerStorageDataset(),
   );
   console.log(suite.successRate); // expect 100
   ```
   The helper internally uses:
   - `BalancerConfigStore.resetToDefault()` for a pristine baseline
   - `BalancerConfigStore.save()` / `BalancerConfigStore.load()` wired through `PersistenceService.saveData/loadData`
   - A verbose `StorageTestFramework` instance named `BalancerConfigStore`
3. **Run the dedicated CLI**
   ```bash
   npm run storage:test:balancer
   ```
   This command (defined in `package.json`) executes `scripts/storage/runStorageMatrix.ts`, prints the suite summary, and writes a detailed JSON log to:
   ```
   test-results/storage/balancer-config-YYYYMMDD.log
   ```
4. **CI / Evidence**
   - Include the generated log in Kanban evidence (`Evidence: test-results/storage/balancer-config-<date>.log`)
   - Keep lint + unit tests in the safeguard bundle:
     ```
     npm run lint -- src/balancing/config/BalancerConfigStore.ts tests/unit/storage/BalancerConfigStore.storage.test.ts
     npm run test:unit -- tests/unit/storage/BalancerConfigStore.storage.test.ts
     npm run storage:test:balancer
     npm run build:check
     npm run kanban:lint
     ```
...
describe('BalancerConfigStore storage matrix', () => {
  it('should pass all storage tests', async () => {
    const primary = buildPrimaryBalancerStorageDataset();
    const alternate = buildAlternateBalancerStorageDataset();
    const suite = await testBalancerConfigStore(primary, alternate);
...
### Dedicated CLI Runner

Run the BalancerConfigStore storage matrix via the purpose-built CLI:

```bash
npm run storage:test:balancer
```

This command executes `scripts/storage/runStorageMatrix.ts`, prints a summary to stdout, and writes a JSON log to `test-results/storage/balancer-config-YYYYMMDD.log`. The log contains the full `TestSuite` payload from `StorageTestFramework` for later inspection or CI evidence.
...
- name: Run Balancer storage matrix
  run: npm run storage:test:balancer
```
