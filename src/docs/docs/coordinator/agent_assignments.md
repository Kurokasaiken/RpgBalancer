...
| ADR001-T1 — Freeze E1 (branching QuestEngine) | Completato | - | Cascade | - | 2026-07-22T13:05:00Z | manual | executed by Cascade | 2026-07-22T13:05:00Z | Prepend @experimental FROZEN JSDoc header to src/engine/quest/QuestEngine.ts. Atomic, independent. Safeguards: lint (ESLint ignored files, no errors), build:check passed, kanban:lint passed. Evidence: test-results/build-check-2026-07-22.log. DoD: header present, no import diff, grep "new QuestEngine(" in src/ yields only tests+docs. Plan: src/docs/docs/plans/quest_engine_reconciliation_plan.md §T1 FILE TARGET: src/engine/quest/QuestEngine.ts |
...
| ADR001-T4 — Purge Math.random() in idleVillage engines | Completato | - | harness | - | 2026-07-17T13:53:06.732Z | harness | - | 2026-07-17T13:53:06.732Z | Replace all Math.random() in JobResolver, QuestPowerEngine, QuestResolver, QuestEngine (E2), TimeEngine (spawnQuestOffersIfNeeded) with injected Rng derived from masterSeed. IDs become deterministic via deriveSeed(masterSeed, 'ids', counter). Verified. Safeguards: lint+test+build:check on src/engine/game/idleVillage. Regression test: grep "Math.random" src/engine/game/idleVillage returns 0. Evidence: test-results/adr001-t4-<date>.log with grep output. DoD: grep clean, no test regression. Plan: src/docs/docs/plans/quest_engine_reconciliation_plan.md §T4 |
 | ADR001-T5 — Migrate C1 quests to C2 (ActivityDefinition) | Completato | - | Cascade | - | - | - | - | 2026-07-16 | Migrated C1 quests to C2 ActivityDefinition entries. Updated defaultConfig.ts, minimal-poi.tsx, questDetailKit.tsx, locationDetailKit.tsx, gameplayStore.ts, questStateDefaults.ts. Evidence: test-results/adr001-t5-2026-07-16.log. Plan: src/docs/docs/plans/quest_engine_reconciliation_plan.md §T5 | 
| ADR001-T6 — Deprecate C1 (questConfig.ts) and E2 | Completato | - | Cascade | - | 2026-07-22T13:05:00Z | manual | executed by Cascade | 2026-07-22T13:05:00Z | Add @deprecated JSDoc + ADR-001 reference to questConfig.ts and QuestEngine.ts (E2). Keep exports. Atomic. Documentation only, no runtime changes. Safeguards: lint (ESLint ignored files, no errors), build:check passed, kanban:lint passed. Evidence: test-results/build-check-2026-07-22.log. DoD: JSDoc present, no new C1 imports after this task. Plan: src/docs/docs/plans/quest_engine_reconciliation_plan.md §T6 FILE TARGET: src/balancing/config/idleVillage/questConfig.ts, src/engine/game/idleVillage/QuestEngine.ts |
...
| POI-MEDALLION-MATERIAL-PREVIEW-00E669 — POI Medallion Material Preview (GenericPoiSkin V2) | Completato | - | Cascade | 2026-07-22T09:56:00Z | 2026-07-22T17:20:00Z | manual | execution_hint architectural, visual design judgment | 2026-07-22T17:20:00Z | Materic stone/bronze POI medallion V2 + /poi-visual-preview A/B page. Canonical GenericPoiSkin untouched. Safeguards: build:check, targeted lint (no-ignore), unit test, kanban:lint passed. Evidence: test-results/POI-MEDALLION-MATERIAL-PREVIEW-2026-07-22.log |
