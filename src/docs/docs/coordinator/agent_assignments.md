...
| ADR001-T1 — Freeze E1 (branching QuestEngine) | Assegnato | 2026-07-22 | ai-worker | Prepend @experimental FROZEN JSDoc header to src/engine/quest/QuestEngine.ts. Atomic, independent. Safeguards: lint src/engine/quest, build:check. Evidence: test-results/adr001-t1-<date>.log. DoD: header present, no import diff, grep "new QuestEngine(" in src/ yields only tests+docs. Plan: src/docs/docs/plans/quest_engine_reconciliation_plan.md §T1 | ai-worker | atomic routing | Prompt: prompts/ADR001-T1.spec.json |
...
| ADR001-T6 — Deprecate C1 (questConfig.ts) and E2 | Assegnato | 2026-07-22 | ai-worker | Add @deprecated JSDoc + ADR-001 reference to questConfig.ts and QuestEngine.ts (E2). Keep exports. Atomic. Documentation only, no runtime changes. Safeguards: lint+build:check. Evidence: test-results/adr001-t6-<date>.log. DoD: JSDoc present, no new C1 imports after this task. Plan: src/docs/docs/plans/quest_engine_reconciliation_plan.md §T6 | ai-worker | atomic routing | Prompt: prompts/ADR001-T6.spec.json |
