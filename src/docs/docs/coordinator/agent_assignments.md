...
| R-066-RIPPLE | Completato | 2026-09-08 | Devin | Director selected option 3: animated sprite-sheet asset. Downloaded OpenGameArt `ripples.7z` (CC0, 30 frames 512x512), converted to 5x6 sprite sheet `public/assets/atmosphere/sea/ripples_sprite.webp` (60KB). Extended `SeaRippleConfig` with `mode: 'smil' | 'sprite'`; default is sprite. `WorldSurfaceSeaRipple` now steps the sheet with CSS keyframes, `mix-blend-mode: overlay`, `shallow_mask.webp` clipping, `prefers-reduced-motion`. SMIL displacement mode kept as fallback. Safeguards: lint 0 errors, 7 tests pass, build:check pass, kanban:lint pass. Smoke: /world-surface 200, /sea-effect-lab 200. Evidence: test-results/r066-sea-ripple-sprite-2026-09-08.log | ai-worker | Director directive to implement chosen lab variant in `WorldSurfaceRenderer` | Evidence: test-results/r066-sea-ripple-sprite-2026-09-08.log |
| R-066-PLAN-013 | Completato | 2026-09-08 | Devin | PLAN-013 Sea Marks: `WorldSurfaceSeaMarks` DOM overlay with 34 transparent hand-painted marks (24 coast + 10 open water), CSS keyframes on `opacity` and `transform: translate3d()`, `prefers-reduced-motion` kill switch, sea_mask clipping, Zod schema in `seaMarksConfig.ts`. Wired into `WorldSurfaceRenderer` via `showSeaMarks` prop. `SeaEffectLabPage` extended with `seamarksCoast`/`seamarksOpen`/`seamarksBoth` A/B variants. Safeguards: lint 0 errors, build:check pass, 7 tests pass, kanban:lint pass. Smoke: /sea-effect-lab 200, /world-surface 200. Evidence: test-results/r066-sea-motion-2026-09-08.log | ai-worker | PLAN-013 implementation executed after multi-AI approval (ChatGPT/Claude/DeepSeek) | Evidence: test-results/r066-sea-motion-2026-09-08.log |
| IV-POI-QUEST-R013 | Completato | 2026-08-14 | Claude | Playwright UI integration tests per `/poi-quest-detail-roster-time-clock`: 17 pass, 1 skip, 0 fallimenti; ERR-005 Start gating chiuso; aggiunta `addResources` nello store e verifica che `QuestRewardPanel.collect` incrementi `gold/food/wood/xp` (page shell, apertura detail con pausa, Start gating, drop invalid, panel drag, bloom POI/slot compatibilità, astrolabe skill check, end-to-end quest + rewards, pause/resume, assegnazione compatibile nel detail). Esposti `__idleVillageTestHooks` (findAcceptingSlot, getResidentCompatibility, setDraggingResidentId). Implementata pausa automatica all'apertura del POI detail. POI detail ripristinato alla reference `PoiDetailVerificationPage` (`/poi-detail-verification`): rimosso `FloatingPanel` wrapper, `ActivityCapsuleDetailSkinAware` usato direttamente con `enableDrag` e `inlineMode: false`. DayNightPOI dotato di data-* attributes (`data-testid`, `data-phase`, `data-paused`, `data-progress`) e testati a runtime (halo avanza, pausa blocca, transizione giorno→notte). Pagina POI collegata al canonical time engine con loop `tick(1000, 'auto')` e sync della `config` nello store. Documentazione workflow, error registry, interaction specs, detail spec e COMPONENT_MASTER_INDEX aggiornate. Mismatch runtime: ERR-014 confermato (active milestone non auto-resolve se QuestChronicle chiusa); ERR-005 chiuso (Start/Embark richiede tempo in moto e non auto-resume). Safeguards: build:check, lint, kanban:lint, vitest pass. Evidence: test-results/poi-quest-detail-roster-time-clock-runtime-2026-08-14.md | manual | Task test UI/esecuzione eseguito con mw-executor e idle-village-task | Evidence: test-results/poi-quest-detail-roster-time-clock-runtime-2026-08-14.md |
| IV-POI-QUEST-R005-V4 | Completato | 2026-08-12 | Claude | POI Quest System desiderata v4: FloatingPanel (pannelli spostabili, riducibili a icona, senza backdrop) per detail/quest card/skill check; ritmo delle fasi (una alla volta, tempo che scorre in mezzo: milestone a 2.0/4.0/6.0s su 3 fasi in 6s verificato a runtime); QuestRewardPanel riscritto sui ruoli di /design-system con test anti-colore-letterale; MilestoneCheckModal reso contenuto puro. Corretti 3 difetti (drag dipendente da setPointerCapture, posizione NaN scartata da React, rect.width con ?? invece di ||). Safeguard: tsc/build:check/kanban:lint/eslint verdi, 124 test passed (26 nuovi), route 200. Evidence: test-results/poi-quest-system-r005-2026-08-11.md | manual | UI multi-file eseguita in sessione con mw-executor | Evidence: test-results/poi-quest-system-r005-2026-08-11.md |
...
ISTRUZIONI
Sei un agente Python/ops. Leggi `src/docs/docs/plans/overnight_safety_runner_plan.md` e `prompts/OPS-OVERNIGHT-001.spec.json` prima di iniziare. Implementa solo file nuovi sotto `scripts/overnight_runner/`; non modificare codice esistente.

OBIETTIVO
Crea un runner fail-closed per eseguire un batch di comandi in sequenza (e opzionalmente in parallelo) con:
- per-task timeout e inactivity/heartbeat timeout;
- global timeout;
- terminazione del process group (SIGTERM -> grace -> SIGKILL);
- progress `[N/TOTAL]`, streaming stdout/stderr, log persistente in `test-results/`;
- exit code affidabile (0 tutto OK, 1 task fallito/timeout/stuck, 2 errore runner/global timeout);
- dry-run con 5 scenari fittizi: completamento, blocco infinito, errore, timeout globale, stuck/inactivity;
- static guard che blocca `shutdown`/`poweroff`/`halt`/`osascript`/`pmset` nei comandi.

FILE TARGET
- `scripts/overnight_runner.py` (entry point CLI)
- `scripts/overnight_runner/__init__.py`
- `scripts/overnight_runner/config.py`
- `scripts/overnight_runner/runner.py`
- `scripts/overnight_runner/default.config.json`
- `scripts/overnight_runner/dryrun.py`

FORBIDDEN
- Non modificare `Coordinator`, `Strategist`, `ProjectRunner`, `Execution Runtime`.
- Non modificare `scripts/shutdownManager/**` o `scripts/run_with_timeout.py`.
- Non chiamare `shutdown`, `poweroff`, `halt`, `osascript -e 'tell application "System Events" to shut down'`, `pmset sleep`, `sudo shutdown -h now` o equivalenti.
- Non creare UI, file `.css`, React Context/Zustand, i18n, persistence app.
- Non introdurre entita canonica (`Execution`, `Attempt`, `Claim`, `Lease`, `Heartbeat`).

SAFEGUARDS
- `python3 scripts/overnight_runner.py --dry-run` deve terminare entro 60s con tutti gli scenari OK.
- `npm run kanban:lint`.
- Grep locale per verificare zero shutdown call sites nel codice creato.
- Evidence log: `test-results/overnight-runner-dryrun-<timestamp>.log`.

NOTE
Per stanotte il runner e un ponte ops, NON il Global Session Shutdown Manager. Scrivi evidence log e summary JSON in `test-results/`. Aggiorna `src/docs/docs/coordinator/strategy_tasks.md` e `src/docs/docs/coordinator/agent_assignments.md` con lo stato `Completato` al termine.
```
...
```
 ...
 | TRAILER-V9-PHASE1 | Completato | Completato | Cascade | Evidence: test-results/trailer-v9-phase1-2026-07-17.log | manual | @trailer-only exemption, SWE/Cascade required | ```text
AGENT
...
OBIETTIVO
Riscrivere `src/ui/idleVillage/trailer/TrailerThreatIter.tsx` usando la mappa corretta, l'estetica teal di Visual Fidelity Lab, i POI esistenti di `/minimal-poi` e una copia del componente POI detail per il pannello titolo/timer.

CONTEXT
L'utente ha confermato:
1. La mappa si chiama `map.jpg` (repo root).
2. La base teal e' quella di `/visual-fidelity-lab` (obsidian #060f16 + azure radial leak in alto).
3. Il pannello titolo/timer va copiato dal POI detail di `/poi-detail-verification` (`ActivityCapsuleDetailSkinAware`) e modificato liberamente.
4. I POI devono essere quelli gia' esistenti su `/minimal-poi` (`GenericPoiSkin` / POI kit).
5. Il route `/trailer-threat-iter` resta invariato.

@trailer-only CONVENTION
EXEMPT:
- NO PersistenceService, NO localStorage/sessionStorage
- NO i18n per copy di trailer (hardcoded ammesso)
- NO telemetry
- NO gameplay mutation, NO economy systems

MUST PRESERVE:
- Config-first: valori in `trailerConfig.ts`
- Component reuse: GenericPoiSkin, WanderlustSurface, WanderlustAmbientField, InsetPanel
- NO standalone .css files
- JSDoc su nuove funzioni/interfacce
- Route invariato
- Safeguards: lint, build:check, kanban:lint

WHAT YOU MUST DO
1. Verifica che `map.jpg` sia raggiungibile dal dev server (copia in `public/` se necessario); aggiorna `trailerConfig.threat.mapImage`.
2. Crea `src/ui/idleVillage/trailer/TrailerThreatDetailPanel.tsx` come fork ridotto di `ActivityCapsuleDetailSkinAware` (solo titolo, sottotitolo, targa, timer; niente slot rack, telemetry, CTA).
3. Riscrivi `src/ui/idleVillage/trailer/TrailerThreatIter.tsx`:
   - Sfondo `map.jpg`.
   - Overlay teal/obsidian di Visual Fidelity Lab (#060f16 + radial gradient azure in alto).
   - POI nodes come `GenericPoiSkin` da `@/ui/idleVillage/frozen/kits/poiKit`, posizionati secondo `trailerConfig.threat.pois`.
   - Pannello centrale con `TrailerThreatDetailPanel` e timer in `InsetPanelDelicate` con testo old-gold e ombre forti.
4. Aggiorna `src/balancing/config/idleVillage/trailerConfig.ts` se servono nuovi token.
5. Verifica `/trailer-threat-iter` senza errori console.

SUCCESS CRITERIA
- /trailer-threat-iter renderizza.
- Sfondo = map.jpg.
- Base = VFL teal/obsidian.
- POI = GenericPoiSkin (non medaglie custom).
- Pannello centrale = copia POI detail con old-gold heraldic text e ombre forti.
- Route invariato.
- Lint/build/kanban pass.
...
SAFEGUARDS
- Lint: `npm run lint -- src/ui/idleVillage/trailer/` (120s)
- Build: `npm run build:check` (180s)
- Kanban: `npm run kanban:lint` (30s)

PLAN REFERENCE
docs/plans/trailer_threat_iter_rework_plan.md

KANBAN UPDATE
Dopo il completamento:
1. Aggiorna questa riga a "Completato" con la data e l'evidence log.
2. Esegui `npm run kanban:lint`.
```

| TRAILER-V9-PHASE2 | Completato | 2026-07-17 | Cascade | Evidence: test-results/trailer-v9-phase2-2026-07-17.log | manual | @trailer-only exemption, SWE/Cascade required | ```text
AGENT
...
| TRAILER-V9-PHASE3 | Completato | 2026-07-17 | Cascade | Evidence: test-results/trailer-v9-phase3-2026-07-17.log | manual | @trailer-only exemption, SWE/Cascade required | ```text
AGENT
...
| TRAILER-V9-PHASE4 | Completato | 2026-07-17 | Cascade | Evidence: test-results/trailer-v9-phase4-2026-07-17.log | manual | @trailer-only exemption, SWE/Cascade required | ```text
AGENT
...
```
| DOC-MG-ROSTER-RECONCILIATION-008 - Optional Post-Fix Documentation Reconciliation | Completato | Cascade | 2026-07-15 | COMPONENT_MASTER_INDEX.md updated with shared bundle reference, roster_drag_trusted.md verified as valid baseline, roster_bundle_usage.md updated with adoption notes. Evidence: test-results/doc-mg-roster-reconciliation-008-2026-07-15.log |
AGENT
...
| ADR001-T1 — Freeze E1 (branching QuestEngine) | Completato | - | Cascade | - | 2026-07-22T13:05:00Z | manual | executed by Cascade | 2026-07-22T13:05:00Z | Prepend @experimental FROZEN JSDoc header to src/engine/quest/QuestEngine.ts. Atomic, independent. Safeguards: lint (ESLint ignored files, no errors), build:check passed, kanban:lint passed. Evidence: test-results/build-check-2026-07-22.log. DoD: header present, no import diff, grep "new QuestEngine(" in src/ yields only tests+docs. Plan: src/docs/docs/plans/quest_engine_reconciliation_plan.md §T1 FILE TARGET: src/engine/quest/QuestEngine.ts. blocked_by: — |
| ADR001-T2 — Extract RngService | Completato | - | harness | - | 2026-07-17T13:53:06.683Z | harness | - | 2026-07-17T13:53:06.677Z | Create src/engine/shared/RngService.ts (LCG + createRng, deriveSeed, rollOutcome), src/engine/shared/rngConfig.ts (Zod WeightedDistribution), tests/unit/engine/shared/RngService.test.ts. Lift LCG from E1, keep E1 importing from new module. Verified. Safeguards: lint src/engine/shared tests/unit/engine/shared, test src/engine/shared, build:check. Evidence: test-results/adr001-t2-<date>.log. DoD: tests green, E1 compiles, no behavioral drift. Plan: src/docs/docs/plans/quest_engine_reconciliation_plan.md §T2. blocked_by: — |
| ADR001-T3 — Persist masterSeed | Completato | - | harness | - | 2026-07-17T13:53:06.711Z | harness | - | 2026-07-17T13:53:06.711Z | Add masterSeed field to Zustand store, initialize once at run creation. Add persistence key idleVillage_master_seed_v1. TimeEngine accepts injected rng derived from masterSeed. Create tests/unit/idleVillage/masterSeed.test.ts. Verified. Safeguards: lint+test+build:check on scope. Evidence: test-results/adr001-t3-<date>.log. DoD: save→load→save produces identical seed, telemetry master_seed_initialized fires once per run. Plan: src/docs/docs/plans/quest_engine_reconciliation_plan.md §T3. blocked_by: ADR001-T2 |
| ADR001-T4 — Purge Math.random() in idleVillage engines | Completato | - | harness | - | 2026-07-17T13:53:06.732Z | harness | - | 2026-07-17T13:53:06.732Z | Replace all Math.random() in JobResolver, QuestPowerEngine, QuestResolver, QuestEngine (E2), TimeEngine (spawnQuestOffersIfNeeded) with injected Rng derived from masterSeed. IDs become deterministic via deriveSeed(masterSeed, 'ids', counter). Verified. Safeguards: lint+test+build:check on src/engine/game/idleVillage. Regression test: grep "Math.random" src/engine/game/idleVillage returns 0. Evidence: test-results/adr001-t4-<date>.log with grep output. DoD: grep clean, no test regression. Plan: src/docs/docs/plans/quest_engine_reconciliation_plan.md §T4. blocked_by: ADR001-T2,ADR001-T3. Proposes permanent grep Math.random CI gate. |
 | ADR001-T5 — Migrate C1 quests to C2 (ActivityDefinition) | Completato | - | Cascade | - | - | - | - | 2026-07-16 | Migrated C1 quests to C2 ActivityDefinition entries. Updated defaultConfig.ts, minimal-poi.tsx, questDetailKit.tsx, locationDetailKit.tsx, gameplayStore.ts, questStateDefaults.ts. Evidence: test-results/adr001-t5-2026-07-16.log. Plan: src/docs/docs/plans/quest_engine_reconciliation_plan.md §T5. blocked_by: ADR001-T2. Trusted doc updates: poi_detail_trusted.md, COMPONENT_MASTER_INDEX.md. | 
| ADR001-T6 — Deprecate C1 (questConfig.ts) and E2 | Completato | - | Cascade | - | 2026-07-22T13:05:00Z | manual | executed by Cascade | 2026-07-22T13:05:00Z | Add @deprecated JSDoc + ADR-001 reference to questConfig.ts and QuestEngine.ts (E2). Keep exports. Atomic. Documentation only, no runtime changes. Safeguards: lint (ESLint ignored files, no errors), build:check passed, kanban:lint passed. Evidence: test-results/build-check-2026-07-22.log. DoD: JSDoc present, no new C1 imports after this task. Plan: src/docs/docs/plans/quest_engine_reconciliation_plan.md §T6 FILE TARGET: src/balancing/config/idleVillage/questConfig.ts, src/engine/game/idleVillage/QuestEngine.ts. blocked_by: ADR001-T5 |
| ADR001-T7 — Complete quality-roll via variance.rewardCategories | Completato | - | harness | - | 2026-07-17T13:53:06.764Z | harness | - | 2026-07-17T13:53:06.764Z | Replace "always first category" stub in QuestResolver.ts (~lines 77-86) with rollOutcome using varianceCategory from ActivityDefinition. Ensure QualityResult { tier, multiplier } type exists in types.ts. Add tests/unit/idleVillage/QuestResolver.test.ts covering roll distribution with seeded RNG. Verified. Safeguards: lint+test+build:check on scope. Evidence: test-results/adr001-t7-<date>.log. DoD: deterministic outcome given same seed, distribution matches config weights within tolerance. Plan: src/docs/docs/plans/quest_engine_reconciliation_plan.md §T7. blocked_by: ADR001-T4 |
| ADR001-T8 — Documentation & governance closure | Completato | - | harness | - | 2026-07-17T13:53:06.787Z | harness | - | 2026-07-17T13:53:06.787Z | Create src/docs/docs/adr/ADR-001-quest-engine-reconciliation.md. Update quest_engine_reconciliation_plan.md with per-task changelog. Cross-link ADR-001 in idle_village_plan.md Quest/Job System section. Mark quest_chronicle_plan.md Phase C theater CTA + sandbox wiring as Step 2 post-reconciliation. Mark idle_village_modifiers_plan.md GM-ENG wiring as post-demo. Add ADR-001 entry to MASTER_PLAN.md governance section. Update COMPONENT_MASTER_INDEX.md with touched frozen kit rows. Verified. Safeguards: kanban:lint, markdown lint if configured. Evidence: test-results/adr001-t8-<date>.log. DoD: all links resolve, COMPONENT_MASTER_INDEX.md last-certified dates updated, master plan references ADR. Plan: src/docs/docs/plans/quest_engine_reconciliation_plan.md §T8. blocked_by: ADR001-T1,ADR001-T6,ADR001-T7 |
...
## Strategist Audit Addendum 2026-07-23 — Kanban Queue

| ID | Status | Data | Agent | Note | Executor | Executor Reason | Prompt |
|---|---|---|---|---|---|---|---|
| IV-PROG-MASTER-001 | Non assegnato | 2026-07-23 | — | Master: Progression System P1-P6 | — | — | Prompt pending: see coordinator_handoff_2026-07-23.md §2 |
| IV-TF-MASTER-001 | Non assegnato | 2026-07-23 | — | Master: Tick & Fatigue System | — | — | Prompt pending: see coordinator_handoff_2026-07-23.md §3 |
| WL-LORE-DROP-001 | Non assegnato | 2026-07-23 | — | Master: Lore Drop Prototype | — | — | Prompt pending: see coordinator_handoff_2026-07-23.md §5 |
| IV-WSC-001 | Non assegnato | 2026-07-23 | — | Master: Component-Based World Surface | — | — | Prompt pending: see coordinator_handoff_2026-07-23.md §6 |
| IV-WSV3-005 | Non assegnato | 2026-07-23 | — | Master: World Surface V3 Phase 5 | — | — | Prompt pending: see coordinator_handoff_2026-07-23.md §7 |
| REND-SYS-001 | Non assegnato | 2026-07-23 | — | Master: Rendering System | — | — | Prompt pending: see coordinator_handoff_2026-07-23.md §8 |
| ART-DIR-001 | Non assegnato | 2026-07-23 | — | Master: Art Direction v0.11 + creature | — | — | Prompt pending: see coordinator_handoff_2026-07-23.md §10 |
| WL-WPG-001 | Non assegnato | 2026-07-23 | — | Master: World Presence Grammar | — | — | Prompt pending: see coordinator_handoff_2026-07-23.md §11 |
| WL-TEASER-001 | Non assegnato | 2026-07-23 | — | Master: Steam Concept Slice | — | — | Prompt pending: see coordinator_handoff_2026-07-23.md §12 |
| CDA-MASTER-001 | Non assegnato | 2026-07-23 | — | Master: Config-Driven Architecture | — | — | Prompt pending: see coordinator_handoff_2026-07-23.md §13 |
| IV-PROG-P1 | Non assegnato | 2026-07-23 | — | Fase 1: XP Progression Formula + Level Table (master IV-PROG-MASTER-001) | manual | verified multi-file config+engine+tests, design judgment | Prompt: prompts/IV-PROG-P1.md |
| IV-TF-001 | Non assegnato | 2026-07-23 | — | Fase 001: Fatigue Config Schema + Recovery Rates (master IV-TF-MASTER-001) | manual | verified config schema, integration with defaultConfig | Prompt: prompts/IV-TF-001.md |
| WL-LORE-DROP-F1 | Non assegnato | 2026-07-23 | — | Fase F1: Lore Drop Types + Sample Pool (master WL-LORE-DROP-001) | manual | verified config types+12 samples, no runtime yet | Prompt: prompts/WL-LORE-DROP-F1.md |
| IV-WSC-P1 | Non assegnato | 2026-07-23 | — | Fase P1: Schemas + Query Engine (master IV-WSC-001) | manual | architectural: world component foundational layer | Prompt: prompts/IV-WSC-P1.md |
| IV-WSV3-P5 | Non assegnato | 2026-07-23 | — | Fase P5: Polish, Calibration & Testing (master IV-WSV3-005) | manual | verified multi-file/E2E, performance/calibration judgment | Prompt: prompts/IV-WSV3-P5.md |
| REND-SYS-P1 | Non assegnato | 2026-07-23 | — | Fase P1: Foundation Schemas + Material/Frame/Recipe Libraries (master REND-SYS-001) | manual | architectural: rendering system foundation | Prompt: prompts/REND-SYS-P1.md |
| ART-DIR-UPDATE-001 | Non assegnato | 2026-07-23 | — | Fase UPDATE-001: Update art direction bible to v0.11 (master ART-DIR-001) | manual | architectural/doc: design bible update | Prompt: prompts/ART-DIR-UPDATE-001.md |
| WL-WPG-001-CONCEPT | Non assegnato | 2026-07-23 | — | Fase CONCEPT: Goblin Invasion 4-Frame Concept + Delta Test (master WL-WPG-001) | manual | architectural/design: concept + matrix draft | Prompt: prompts/WL-WPG-001-CONCEPT.md |
| WL-TEASER-D1 | Non assegnato | 2026-07-23 | — | Fase D1: Scaffolding route/controller/config (master WL-TEASER-001) | manual | verified trailer-only scaffolding | Prompt: prompts/WL-TEASER-D1.md |
| CDA-00-001 | Non assegnato | 2026-07-23 | — | Fase 00-001: Architecture Foundation & ADRs (master CDA-MASTER-001) | manual | architectural: ADRs + foundation plan | Prompt: prompts/CDA-00-001.md
| ADR001-T1 | Non assegnato | 2026-07-23 | — | Freeze E1 (branching QuestEngine) | manual | verified: atomic independent task | Prompt: prompts/ADR001-T1.md
| ADR001-T2 | Non assegnato | 2026-07-23 | — | Extract RngService | manual | verified: deterministic RNG service | Prompt: prompts/ADR001-T2.md
| ADR001-T3 | Non assegnato | 2026-07-23 | — | Persist masterSeed | manual | verified: persistence via PersistenceService; blocked_by: ADR001-T2 | Prompt: prompts/ADR001-T3.md
| ADR001-T4 | Non assegnato | 2026-07-23 | — | Purge Math.random() from quest engine | manual | verified: grep + replace + CI gate; blocked_by: ADR001-T2,ADR001-T3 | Prompt: prompts/ADR001-T4.md
| ADR001-T6 | Non assegnato | 2026-07-23 | — | Quest telemetry + outcome parity tests | manual | verified: telemetry + parity tests; blocked_by: ADR001-T2,ADR001-T3 | Prompt: prompts/ADR001-T6.md
| ADR001-T7 | Non assegnato | 2026-07-23 | — | Complete quality-roll via variance.rewardCategories | manual | verified: deterministic outcome per seed; blocked_by: ADR001-T4 | Prompt: prompts/ADR001-T7.md
| ADR001-T8 | Non assegnato | 2026-07-23 | — | Documentation & governance closure | manual | verified: trusted docs + COMPONENT_MASTER_INDEX; blocked_by: ADR001-T1,ADR001-T6,ADR001-T7 | Prompt: prompts/ADR001-T8.md
| IV-PROG-P2 | Non assegnato | 2026-07-23 | — | Fase P2: Reward and Risk Scaling (master IV-PROG-MASTER-001) | manual | verified config+engine+tests; blocked_by: IV-PROG-P1 | Prompt: prompts/IV-PROG-P2.md
| IV-PROG-P3 | Non assegnato | 2026-07-23 | — | Fase P3: Effective Power Calculator (master IV-PROG-MASTER-001) | manual | verified config+engine+tests; blocked_by: IV-PROG-P2 | Prompt: prompts/IV-PROG-P3.md
| IV-PROG-P4 | Non assegnato | 2026-07-23 | — | Fase P4: Production Scaling Integration (master IV-PROG-MASTER-001) | manual | verified engine extension+config; blocked_by: IV-PS0,IV-PROG-P1 | Prompt: prompts/IV-PROG-P4.md
| IV-PROG-P5 | Non assegnato | 2026-07-23 | — | Fase P5: Telemetry + UI Integration (master IV-PROG-MASTER-001) | manual | verified telemetry+UI+tests; blocked_by: IV-PROG-P3,IV-PROG-P4 | Prompt: prompts/IV-PROG-P5.md
| IV-PROG-P6 | Non assegnato | 2026-07-23 | — | Fase P6: End-to-End Tests + Plan Update (master IV-PROG-MASTER-001) | manual | verified E2E+plan update; blocked_by: IV-PROG-P1,IV-PROG-P2,IV-PROG-P3,IV-PROG-P4,IV-PROG-P5 | Prompt: prompts/IV-PROG-P6.md
| IV-TF-002 | Non assegnato | 2026-07-23 | — | Fase 002: Tick Engine Fatigue Application (master IV-TF-MASTER-001) | manual | verified engine extension+fatigue application; blocked_by: IV-TF-001 | Prompt: prompts/IV-TF-002.md
| IV-TF-003 | Non assegnato | 2026-07-23 | — | Fase 003: Resident Fatigue State + Recovery (master IV-TF-MASTER-001) | manual | verified store extension+recovery logic; blocked_by: IV-TF-002 | Prompt: prompts/IV-TF-003.md
| IV-TF-004 | Non assegnato | 2026-07-23 | — | Fase 004: UI Hooks (Fatigue Badges, Rest Actions) (master IV-TF-MASTER-001) | manual | verified UI hooks+i18n+tests; blocked_by: IV-TF-003 | Prompt: prompts/IV-TF-004.md
| IV-TF-005 | Non assegnato | 2026-07-23 | — | Fase 005: Modifier Integration + Tests (master IV-TF-MASTER-001) | manual | verified modifier integration+tests; blocked_by: IV-TF-002,GM-ENG | Prompt: prompts/IV-TF-005.md
| WL-LORE-001-A | Non assegnato | 2026-07-23 | — | Fase A: Lore Config Schema + Sample Entries (master WL-LORE-001) | manual | verified config schema+sample entries | Prompt: prompts/WL-LORE-001-A.md
| WL-LORE-001-B | Non assegnato | 2026-07-23 | — | Fase B: LoreDiscoveryService with Triggers (master WL-LORE-001) | manual | verified service+triggers+tests; blocked_by: WL-LORE-001-A | Prompt: prompts/WL-LORE-001-B.md
| WL-LORE-001-C | Non assegnato | 2026-07-23 | — | Fase C: LoreStore with PersistenceService (master WL-LORE-001) | manual | verified store+PersistenceService+tests; blocked_by: WL-LORE-001-B | Prompt: prompts/WL-LORE-001-C.md
| WL-LORE-001-D | Non assegnato | 2026-07-23 | — | Fase D: LoreBook UI Components (master WL-LORE-001) | manual | verified UI components+tests; blocked_by: WL-LORE-001-C | Prompt: prompts/WL-LORE-001-D.md
| WL-LORE-001-E | Non assegnato | 2026-07-23 | — | Fase E: Gameplay Integration (Quest/Curio/Location) (master WL-LORE-001) | manual | verified gameplay integration+i18n; blocked_by: WL-LORE-001-B,WL-LORE-001-D | Prompt: prompts/WL-LORE-001-E.md
| WL-LORE-DROP-F2 | Non assegnato | 2026-07-23 | — | Fase F2: LoreDropService + Store (master WL-LORE-DROP-001) | manual | verified service+store+tests; blocked_by: WL-LORE-DROP-F1 | Prompt: prompts/WL-LORE-DROP-F2.md
| WL-LORE-DROP-F3 | Non assegnato | 2026-07-23 | — | Fase F3: Wire into QuestChronicle (master WL-LORE-DROP-001) | manual | verified QuestChronicle integration+hooks; blocked_by: WL-LORE-DROP-F2 | Prompt: prompts/WL-LORE-DROP-F3.md
| WL-LORE-DROP-F4 | Non assegnato | 2026-07-23 | — | Fase F4: Optional Location/Building Drops (master WL-LORE-DROP-001) | manual | verified location/building integration; blocked_by: WL-LORE-DROP-F3 | Prompt: prompts/WL-LORE-DROP-F4.md
| WL-LORE-DROP-F5 | Non assegnato | 2026-07-23 | — | Fase F5: In-Game Smoke Test (master WL-LORE-DROP-001) | manual | verified manual QA on /minimal-gameplay; blocked_by: WL-LORE-DROP-F3 | Prompt: prompts/WL-LORE-DROP-F5.md
| WL-LORE-DROP-F6 | Non assegnato | 2026-07-23 | — | Fase F6: Evidence Log + Plan Status (master WL-LORE-DROP-001) | manual | verified plan update+evidence log; blocked_by: WL-LORE-DROP-F1,WL-LORE-DROP-F2,WL-LORE-DROP-F3,WL-LORE-DROP-F4,WL-LORE-DROP-F5 | Prompt: prompts/WL-LORE-DROP-F6.md
| IV-WSC-P2 | Non assegnato | 2026-07-23 | — | Fase P2: Manifest v1 → v2 Migration (master IV-WSC-001) | manual | architectural: migration system; blocked_by: IV-WSC-P1 | Prompt: prompts/IV-WSC-P2.md
| IV-WSC-P3 | Non assegnato | 2026-07-23 | — | Fase P3: Component Batch Renderer (master IV-WSC-001) | manual | architectural: batch rendering; blocked_by: IV-WSC-P2 | Prompt: prompts/IV-WSC-P3.md
| IV-WSC-P4 | Non assegnato | 2026-07-23 | — | Fase P4: PresentationOutput Extensions (master IV-WSC-001) | manual | architectural: presentation extensions; blocked_by: WORLD-PRESENTATION-RUNTIME-FOUNDATION,IV-WSC-P3 | Prompt: prompts/IV-WSC-P4.md
| IV-WSC-P5 | Non assegnato | 2026-07-23 | — | Fase P5: Test Harness + Director (master IV-WSC-001) | manual | verified test harness+director; blocked_by: IV-WSC-P4 | Prompt: prompts/IV-WSC-P5.md
| IV-WSC-P6 | Non assegnato | 2026-07-23 | — | Fase P6: Governance Close-Out (master IV-WSC-001) | manual | verified trusted docs+COMPONENT_MASTER_INDEX; blocked_by: IV-WSC-P1,IV-WSC-P2,IV-WSC-P3,IV-WSC-P4,IV-WSC-P5 | Prompt: prompts/IV-WSC-P6.md
| REND-SYS-P2 | Non assegnato | 2026-07-23 | — | Fase P2: React Components + V9PanelShell Refactor (master REND-SYS-001) | manual | architectural: component refactor; blocked_by: REND-SYS-P1 | Prompt: prompts/REND-SYS-P2.md
| REND-SYS-P3 | Non assegnato | 2026-07-23 | — | Fase P3: Library Expansion (master REND-SYS-001) | manual | architectural: library expansion; blocked_by: REND-SYS-P1 | Prompt: prompts/REND-SYS-P3.md
| REND-SYS-P4 | Non assegnato | 2026-07-23 | — | Fase P4: Visual Recipes + Decoration Packs (master REND-SYS-001) | manual | architectural: visual recipes; blocked_by: REND-SYS-P2,REND-SYS-P3 | Prompt: prompts/REND-SYS-P4.md
| REND-SYS-P5 | Non assegnato | 2026-07-23 | — | Fase P5: Art Direction Integration (master REND-SYS-001) | manual | architectural: art direction integration; blocked_by: REND-SYS-P4 | Prompt: prompts/REND-SYS-P5.md
| REND-SYS-P6 | Non assegnato | 2026-07-23 | — | Fase P6: Performance & Optimization (master REND-SYS-001) | manual | architectural: performance optimization; blocked_by: REND-SYS-P2,REND-SYS-P4 | Prompt: prompts/REND-SYS-P6.md
| REND-SYS-P7 | Non assegnato | 2026-07-23 | — | Fase P7: Documentation + Migration Guide (master REND-SYS-001) | manual | verified documentation+guide; blocked_by: REND-SYS-P5,REND-SYS-P6 | Prompt: prompts/REND-SYS-P7.md
| REND-SYS-P8 | Non assegnato | 2026-07-23 | — | Fase P8: Testing & Validation (master REND-SYS-001) | manual | verified tests+visual regression; blocked_by: REND-SYS-P6 | Prompt: prompts/REND-SYS-P8.md
| TRAILER-THREAT-V3-P1 | Non assegnato | 2026-07-23 | — | Fase P1: 4 Static Frames (master TRAILER-THREAT-ITER-V3) | manual | verified 4 static frames | Prompt: prompts/TRAILER-THREAT-V3-P1.md
| TRAILER-THREAT-V3-P2 | Non assegnato | 2026-07-23 | — | Fase P2: Static-to-Static Transition (master TRAILER-THREAT-ITER-V3) | manual | verified transition Expanded ↔ Compact; blocked_by: TRAILER-THREAT-V3-P1 | Prompt: prompts/TRAILER-THREAT-V3-P2.md
| TRAILER-THREAT-V3-P3 | Non assegnato | 2026-07-23 | — | Fase P3: Optional Polish (master TRAILER-THREAT-ITER-V3) | manual | verified polish features; blocked_by: TRAILER-THREAT-V3-P2 | Prompt: prompts/TRAILER-THREAT-V3-P3.md
| ART-DIR-CREATURE-001 | Non assegnato | 2026-07-23 | — | Fase CREATURE-001: Create Canonical Creature Example (master ART-DIR-001) | manual | verified creature example; blocked_by: ART-DIR-UPDATE-001 | Prompt: prompts/ART-DIR-CREATURE-001.md
| WL-WPG-001-CONFIG | Non assegnato | 2026-07-23 | — | Fase CONFIG: Presence Grammar Config + Validation Matrix (master WL-WPG-001) | manual | verified config+validation matrix; blocked_by: WL-WPG-001-CONCEPT | Prompt: prompts/WL-WPG-001-CONFIG.md
| WL-TEASER-D2 | Non assegnato | 2026-07-23 | — | Fase D2: Scene Composites (Map, Village, Choice) (master WL-TEASER-001) | manual | verified scene composites; blocked_by: WL-TEASER-D1 | Prompt: prompts/WL-TEASER-D2.md
| WL-TEASER-D3 | Non assegnato | 2026-07-23 | — | Fase D3: Hero, Drag & Astrolabe (master WL-TEASER-001) | manual | verified hero+drag+astrolabe; blocked_by: WL-TEASER-D2 | Prompt: prompts/WL-TEASER-D3.md
| WL-TEASER-D4 | Non assegnato | 2026-07-23 | — | Fase D4: Consequence, Legacy, Outro (master WL-TEASER-001) | manual | verified consequence+legacy+outro; blocked_by: WL-TEASER-D3 | Prompt: prompts/WL-TEASER-D4.md
| WL-TEASER-D5 | Non assegnato | 2026-07-23 | — | Fase D5: Polish & Controls (master WL-TEASER-001) | manual | verified polish+controls; blocked_by: WL-TEASER-D4 | Prompt: prompts/WL-TEASER-D5.md
| CDA-00-002 | Non assegnato | 2026-07-23 | — | Fase 00-002: Config Schema & Validation ADR (master CDA-MASTER-001) | manual | architectural: config schema ADR; blocked_by: CDA-00-001 | Prompt: prompts/CDA-00-002.md
| CDA-00-003 | Non assegnato | 2026-07-23 | — | Fase 00-003: Component Contract ADR (master CDA-MASTER-001) | manual | architectural: component contract ADR; blocked_by: CDA-00-001 | Prompt: prompts/CDA-00-003.md
| CDA-00-004 | Non assegnato | 2026-07-23 | — | Fase 00-004: Runtime Binding ADR (master CDA-MASTER-001) | manual | architectural: runtime binding ADR; blocked_by: CDA-00-001 | Prompt: prompts/CDA-00-004.md
| CDA-00-005 | Non assegnato | 2026-07-23 | — | Fase 00-005: Skin Contract ADR (master CDA-MASTER-001) | manual | architectural: skin contract ADR; blocked_by: CDA-00-001 | Prompt: prompts/CDA-00-005.md
| CDA-00-006 | Non assegnato | 2026-07-23 | — | Fase 00-006: i18n Contract ADR (master CDA-MASTER-001) | manual | architectural: i18n contract ADR; blocked_by: CDA-00-001 | Prompt: prompts/CDA-00-006.md
| CDA-00-007 | Non assegnato | 2026-07-23 | — | Fase 00-007: Persistence Contract ADR (master CDA-MASTER-001) | manual | architectural: persistence contract ADR; blocked_by: CDA-00-001 | Prompt: prompts/CDA-00-007.md
| CDA-01-001 | Non assegnato | 2026-07-23 | — | Fase 01-001: Component Runtime (master CDA-MASTER-001) | manual | architectural: component runtime; blocked_by: CDA-00-001 | Prompt: prompts/CDA-01-001.md
| CDA-02-001 | Non assegnato | 2026-07-23 | — | Fase 02-001: Rendering Primitive System (master CDA-MASTER-001) | manual | architectural: rendering primitives; blocked_by: CDA-01-001 | Prompt: prompts/CDA-02-001.md
| CDA-03-001 | Non assegnato | 2026-07-23 | — | Fase 03-001: Material Engine (master CDA-MASTER-001) | manual | architectural: material engine; blocked_by: CDA-02-001 | Prompt: prompts/CDA-03-001.md
| CDA-04-001 | Non assegnato | 2026-07-23 | — | Fase 04-001: Physics System (master CDA-MASTER-001) | manual | architectural: physics system; blocked_by: CDA-01-001 | Prompt: prompts/CDA-04-001.md
| CDA-05-001 | Non assegnato | 2026-07-23 | — | Fase 05-001: Seed / Procedural Variation (master CDA-MASTER-001) | manual | architectural: seed system; blocked_by: CDA-01-001 | Prompt: prompts/CDA-05-001.md
| CDA-06-001 | Non assegnato | 2026-07-23 | — | Fase 06-001: Village Evolution System (master CDA-MASTER-001) | manual | architectural: village evolution; blocked_by: CDA-02-001,CDA-04-001,CDA-05-001 | Prompt: prompts/CDA-06-001.md
| CDA-07-001 | Non assegnato | 2026-07-23 | — | Fase 07-001: POI World Map System (master CDA-MASTER-001) | manual | architectural: POI system; blocked_by: CDA-01-001,CDA-02-001,CDA-03-001,CDA-04-001,CDA-05-001 | Prompt: prompts/CDA-07-001.md
| CDA-08-001 | Non assegnato | 2026-07-23 | — | Fase 08-001: Frozen Kit Migration (master CDA-MASTER-001) | manual | architectural: frozen kit migration; blocked_by: CDA-01-001,CDA-02-001,CDA-03-001,CDA-04-001,CDA-05-001,CDA-06-001,CDA-07-001 | Prompt: prompts/CDA-08-001.md
| CDA-09-001 | Non assegnato | 2026-07-23 | — | Fase 09-001: Modding Layer (master CDA-MASTER-001) | manual | architectural: modding layer; blocked_by: CDA-01-001,CDA-02-001,CDA-03-001,CDA-04-001,CDA-05-001,CDA-06-001,CDA-07-001,CDA-08-001 | Prompt: prompts/CDA-09-001.md
| CDA-10-001 | Non assegnato | 2026-07-23 | — | Fase 10-001: AI Production Pipeline (master CDA-MASTER-001) | manual | architectural: AI pipeline; blocked_by: CDA-00-001,CDA-01-001,CDA-02-001,CDA-03-001,CDA-04-001,CDA-05-001,CDA-06-001,CDA-07-001,CDA-08-001,CDA-09-001 | Prompt: prompts/CDA-10-001.md |
| R-070-V63-GOO | Completato | 2026-09-10 | Devin | V6.3 goo Venom/tentacoli: tail nodes per seed drops + domain warp nello shader. Token Zod in `tarGooConfig.ts` (coda, wiggle, wander, domain warp, maxBlobs=56). Fuso head/tail in metaballs, moto deterministico e smorzato; silhouette meno circolari. `prefers-reduced-motion` disabilita warp e wiggle. Safeguards: build pass, `npm run test -- tests/unit/destinyAstrolabeV62/tarGoo.test.ts` 4/4 pass, kanban:lint pass. Smoke: `/minimal-destiny-astrolabe-v6-3` 200 e render OK. Evidence: test-results/v63-goo-venom-poc-evidence-2026-09-10.md | ai-worker | Executor esplicito del Director su R-070 (procedi con l'executor) | Evidence: test-results/v63-goo-venom-poc-evidence-2026-09-10.md |