...
| R-066-RIPPLE | Completato | 2026-09-08 | Devin | Director selected the `rippleSoft` variant for the real map. Added `WorldSurfaceSeaRipple` component: masked copy of `Mare.webp` with SMIL `feTurbulence` + `feDisplacementMap`, clipped by `shallow_mask.webp`, config-first in `atmosphereAssets.seaRipple`, `prefers-reduced-motion` support. Mounted in `WorldSurfaceRenderer` under cloud shadows. Safeguards pending. | ai-worker | Director directive to implement chosen lab variant in `WorldSurfaceRenderer` | TBD |
