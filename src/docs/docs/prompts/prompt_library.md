...
## GT-3 Checklist playtest Punch Club mobile-first (Coordinator Prompt)

**AGENT:** Cascade  
**OBIETTIVO:** Formalizzare checklist playtest mobile Punch Club (loop Gym→Rest→Bout) dentro strategy/idle_village_punch_club_vision.md, integrando metriche Mind Studios (CTA chiare, tap ≤3, risk telemetry leggibile) e collegando flusso ai preset config-first.  

**FILE TARGET:**  
src/docs/docs/strategy/idle_village_punch_club_vision.md (§5, nuova sottosezione "Playtest Checklist – Punch Club Mobile")  
docs/prompts/prompt_library.md (entry aggiornata per riferimento storico)  
docs/tests/PLAYWRIGHT_GUIDE.md (appendice mobile checklist, sezione Punch Club)  
scripts/mobilePlaytestLogger.ts (eventuale nota su nuovi tag telemetry)  

**OPERAZIONI DA ESEGUIRE:**  
1. Kanban: esegui npm run kanban:lint e marca GT-3 come "In progress" in docs/docs/coordinator/agent_assignments.md.  
2. Vision Doc: in strategy/idle_village_punch_club_vision.md aggiungi sottosezione §5.x con metriche obbligatorie (tempo ciclo completo, #tap su picker, delta gold/food, risk stripe reading) e checklist step-by-step (Setup device, consent logger, run Gym→Rest→Bout, esporta log).  
3. Prompt Library: registra questa istruzione in docs/prompts/prompt_library.md seguendo template (AGENT→NOTE).  
4. QA Guide: aggiorna docs/tests/PLAYWRIGHT_GUIDE.md con checklist mobile (sezione "Punch Club Mobile Playtest") e link al logger + tag richiesti.  
5. Logger Script: se mancano tag citati (es. session_tag, cta_latency_ms), documenta TODO inline o apri issue; indica nel prompt se già presenti.  
6. Verifica: npm run lint -- strategy/idle_village_punch_club_vision.md docs/tests/PLAYWRIGHT_GUIDE.md e assicurati Playwright guide build passi (npm run test -- src/ui/punchClub).  
7. Chiusura Kanban: aggiorna GT-3 a "Completed", cita commit/doc modificati e incolla link alla sezione checklist.  

**OUTPUT ATTESI:**  
Vision doc con nuova sezione "Playtest Checklist – Punch Club Mobile" (metriche + passaggi)  
Prompt library aggiornata con entry GT-3  
PLAYWRIGHT_GUIDE sincronizzata con checklist mobile  
Kanban lint pulito e task GT-3 marcato "Completed" con commento al lavoro svolto  

**NOTE:**  
Valida ID univoco prima di salvare nel prompt library.  
Nessun hardcode: tutte le metriche devono riferirsi a preset/statWeight esistenti o telemetria logger.
