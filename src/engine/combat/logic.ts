...
import type { EffectedCharacter, ShieldEffect } from '../../balancing/statusEffects/StatusEffectManager';
...
type NumericStatKey = {
    [K in StatKey]: typeof DEFAULT_STATS[K] extends number ? K : never;
}[StatKey];

function isNumericStatKey(key: StatKey): key is NumericStatKey {
    return typeof DEFAULT_STATS[key] === 'number';
}
...
export function resolveCombatRound(state: CombatState, rng: RNG): CombatState {
    ...
    for (const { entity, team } of allEntities) {
                ...
                if (rng() < 0.5) {
                    ...
                    if (spell.type === 'buff' || spell.type === 'debuff') {
                        ...
                        if (spell.targetStat) {
                            ...
                            const statKey = spell.targetStat as StatKey;
                            if (isNumericStatKey(statKey)) {
                                statChanges[statKey] = value;
                            }
                        }
        ...
        if (attackLanded) {
            ...
            // Filter for shields
            const shields = targetEffectsList.filter(
                (effect): effect is ShieldEffect => effect.type === 'shield'
            );
            let absorbedTotal = 0;
            ...
            if (shields.length > 0) {
                for (const shield of shields) {
                    ...
                    const availableShield = shield.currentShield ?? shield.shieldAmount ?? 0;
                    if (availableShield <= 0) continue;

                    const absorb = Math.min(damageToHp, availableShield);
                    const remainingShield = availableShield - absorb;

                    shield.currentShield = remainingShield;
                    if (typeof shield.shieldAmount === 'number') {
                        shield.shieldAmount = remainingShield;
                    }
                ...
                const activeEffects = targetEffectsList.filter(effect => {
                    if (effect.type !== 'shield') return true;
                    const shield = effect as ShieldEffect;
                    const remaining = shield.currentShield ?? shield.shieldAmount ?? 0;
                    return remaining > 0;
                });
