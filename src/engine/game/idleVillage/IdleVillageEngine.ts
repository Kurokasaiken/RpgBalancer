...
export function tickIdleVillage(deps: IdleVillageEngineDeps, state: VillageState, delta: number): IdleVillageTickResult {
  ...
  for (const scheduledId of completedActivityIds) {
    ...
    if (activity.tags.includes('job')) {
      ...
      const firstEvent = jobResult.events[0];
      const resourceChanges: ResourceDeltaDefinition[] = Array.isArray(firstEvent?.payload?.rewards)
        ? firstEvent.payload.rewards
        : [];

      if (firstEvent) {
        firstEvent.payload = firstEvent.payload ?? { rewards: [] };
        if (!Array.isArray(firstEvent.payload.rewards)) {
          firstEvent.payload.rewards = resourceChanges;
        }
      }
      ...
      if (payload) {
        const questRewards = Array.isArray(payload.rewards) ? payload.rewards : [];
        if (!Array.isArray(payload.rewards)) {
          payload.rewards = questRewards;
        }
