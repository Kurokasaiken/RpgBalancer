 ...
 * Tests the reusable DestinyAstrolabeV6Component with various configurations
 */

import React, { useRef, useState } from 'react';
import { DestinyAstrolabeV6Standalone } from '@/ui/idleVillage/frozen/kits/destinyAstrolabeV6Kit';
import type { AstrolabeResult as DestinyAstrolabeV6Result, AstrolabeSkill as DestinyAstrolabeV6Skill, DestinyAstrolabeV6Handle } from '@/ui/idleVillage/frozen/kits/destinyAstrolabeV6Kit';
...
export default function MinimalDestinyAstrolabeV6() {
  const astrolabeRef = useRef<DestinyAstrolabeV6Handle>(null);
  const [lastResult, setLastResult] = useState<DestinyAstrolabeV6Result | null>(null);
  const [skills, setSkills] = useState<DestinyAstrolabeV6Skill[]>(SKILL_PRESETS.single);
  ...
  const handleComplete = (result: DestinyAstrolabeV6Result) => {
    setLastResult(result);
    console.log('Skill check completed:', result);
  };

  const updateSkill = (idx: number, patch: Partial<DestinyAstrolabeV6Skill>) => {
      ...
      <div className="flex items-center justify-between px-6 py-3 flex-shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div className="flex gap-2">
            ...
            <span className="px-3 py-1 rounded text-sm font-semibold bg-amber-500 text-black">V6</span>
            <a
