 ...
 * Tests the reusable DestinyAstrolabeV9Component with various configurations
 */

import React, { useRef, useState } from 'react';
import { DestinyAstrolabeV9Standalone } from '@/ui/idleVillage/frozen/kits/destinyAstrolabeV9Kit';
import type { AstrolabeResult as DestinyAstrolabeV9Result, AstrolabeSkill as DestinyAstrolabeV9Skill, DestinyAstrolabeV9Handle } from '@/ui/idleVillage/frozen/kits/destinyAstrolabeV9Kit';
...
export default function MinimalDestinyAstrolabeV9() {
  const astrolabeRef = useRef<DestinyAstrolabeV9Handle>(null);
  const [lastResult, setLastResult] = useState<DestinyAstrolabeV9Result | null>(null);
  const [skills, setSkills] = useState<DestinyAstrolabeV9Skill[]>(SKILL_PRESETS.single);
  ...
  const handleComplete = (result: DestinyAstrolabeV9Result) => {
    setLastResult(result);
    console.log('Skill check completed:', result);
  };

  const updateSkill = (idx: number, patch: Partial<DestinyAstrolabeV9Skill>) => {
      ...
      <div className="flex items-center justify-between px-6 py-3 flex-shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div className="flex gap-2">
            ...
            <span className="px-3 py-1 rounded text-sm font-semibold bg-amber-500 text-black">V9</span>
            <a
