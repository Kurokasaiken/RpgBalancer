...
import { createBalancedAllocation } from '../../../balancing/archetype/allocationDefaults';
...
const DEFAULT_ALLOCATION = createBalancedAllocation();
...
export const ArchetypeBuilder: React.FC = () => {
    ...
    const allocationDelta = 100 - allocationSum;
    const deltaBadgeLabel =
        allocationDelta === 0
            ? 'Perfetto'
            : allocationDelta > 0
                ? `+${allocationDelta.toFixed(1)}% liberi`
                : `${allocationDelta.toFixed(1)}% oltre`;
    const deltaBadgeTone =
        allocationDelta >= 0
            ? 'bg-[#102021]/90 text-[#8db3a5] border-[#2d3a38]'
            : 'bg-[#2a1313]/90 text-[#f58c8c] border-[#4a2020]';
                        ...
                        <div className={gildedCard}>
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <div className="text-right">
                                    ...
                                    <div
                                        className={`mt-1 inline-flex items-center gap-1 rounded-full border px-3 py-1 text-[11px] font-semibold uppercase tracking-wide ${deltaBadgeTone}`}
                                    >
                                        <span className="font-mono">{deltaBadgeLabel}</span>
                                    </div>
                                </div>
                            </div>
