...
test.describe('Test Route Drag Offset & Visual QA', () => {
  ...
  test('drag preview canvas centering within 8px of cursor', async ({ page }) => {
    ...
    const slotContainer = page.locator(`[role="listitem"][data-slot-id="${slotId}"]`).first();
    ...
    await dragElement(page, residentCard, targetSlot, {
      ...
      onIntermediateMove: async ({ page, current }) => {
        ...
        if (Math.abs(current.x - targetCenter.x) < 50 && Math.abs(current.y - targetCenter.y) < 50) {
          await expect(slotContainer).toHaveAttribute('data-drop-state', 'valid');
        }
      },
    });
  ...
  test('invalid drop rejection - no assignment occurs', async ({ page }) => {
    // Find first resident card
    const residentCard = page.getByTestId('pg-card').first();
    
    // Try to drop on restricted slot (requires HP ≥ 200)
    // The slot ID is built as: slot-lab-{scenarioId}-slot-{index}
    const restrictedSlotId = 'slot-lab-restricted-slot-0';
    const restrictedSlot = page.getByTestId(`slot-button-${restrictedSlotId}`);
    const restrictedSlotContainer = page.locator(`[role="listitem"][data-slot-id="${restrictedSlotId}"]`).first();
    ...
    // Perform drag to restricted slot
    await page.mouse.move(targetCenter.x, targetCenter.y);
    await page.waitForTimeout(100);
    await page.mouse.up();
    
    // Verify no assignment occurred - this is the core fix we're testing
    await expect(page.getByText('Rack B · assegnato')).toHaveCount(0);
    await expect(page.getByText('Rack A · assegnato')).toHaveCount(0);

    // And ensure the restricted slot still reports invalid or idle drop state
    const dropState = await restrictedSlotContainer.getAttribute('data-drop-state');
    expect(dropState === 'invalid' || dropState === 'idle').toBe(true);
    
    // Optional: Check for any error message (may not appear yet)
