/**
 * Drag & Drop Comprehensive Test
 * 
 * Test completo che esegue tutte le verifiche drag & drop
 * e fornisce un report dettagliato dei risultati.
 */
...
const seedResidents = async (page: Page) => {
  await page.goto('/');
  ...
  await page.evaluate(
    ...
    { key: CHARACTER_STORAGE_KEY, dataset: TEST_RESIDENTS },
  );
};

const ensureResidents = async (page: Page): Promise<number> => {
  ...
  console.log('📦 Auto-seeding Character Manager for comprehensive test');
  await seedResidents(page);
...
const getSlotContainer = (page: Page, slotId: string) => page.locator(`[data-slot-id="${slotId}"][role="listitem"]`);

test('🎯 Drag & Drop Comprehensive Test', async ({ page }) => {
  console.log('\n🎯 DRAG & DROP COMPREHENSIVE TEST');
  ...
  // Inizializzazione con mock data se Character Manager è vuoto
  ...
  // Controlla se ci sono residenti, altrimenti mostra errore esplicativo
  const residentCards = page.getByTestId('pg-card');
  const cardCount = await ensureResidents(page);
  ...
  try {
    ...
    const slotContainer = getSlotContainer(page, slotId);
  ...
  try {
    ...
    const slotContainerLocator = getSlotContainer(page, slotId);
  ...
  try {
    ...
    const restrictedContainer = getSlotContainer(page, slotId);

    await dragElement(page, residentCard3, restrictedSlot, { steps: 14 });
    await page.waitForTimeout(500);
    ...
    if (!hasAssignment) {
      results.invalidDrop = {
        passed: true,
        error: undefined,
        details: 'Drop invalido rifiutato correttamente'
      };
    } else {
      results.invalidDrop = {
        passed: false,
  ...
  try {
    ...
    const rosterCards = page.getByTestId('pg-card');
    ...
    const firstAssignment = await clearButtons.count();

    let assignmentsAfterSecond = firstAssignment;
    if (cardCount > 1) {
      const secondCard = rosterCards.nth(1);
      await secondCard.click();
      await page.waitForTimeout(400);
      assignmentsAfterSecond = await clearButtons.count();
    }

    if (firstAssignment >= 1 && (cardCount === 1 || assignmentsAfterSecond >= firstAssignment + 1)) {
  ...
  try {
    ...
    await dragElement(page, residentCard4, targetSlot4, { steps: 14 });
