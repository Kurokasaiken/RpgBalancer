...
const createInitialState = (config: MinimalConfig) => ({
  ...
  residents: config.startingResources.residents.map((r) => ({
    id: r.id,
    name: r.name,
    stats: r.stats,
    fatigue: r.fatigue,
...
const minimalGameplayStoreInitializer: StateCreator<MinimalGameplayState> = (set, get) => ({
  ...
  tick: (deltaMs, source = 'auto') => {
    ...
    const speedMultiplier = Math.max(0, state.speedMultiplier || 1);
    const deltaTimeUnits = (deltaSeconds * speedMultiplier) / secondsPerTimeUnit;

    if (!Number.isFinite(deltaTimeUnits) || deltaTimeUnits <= 0) {
      return;
    }

    set((s) => {
      const nextState = { ...s.state };
      const dayLengthUnits = Math.max(1, config.globalRules.dayLengthInTimeUnits ?? 60);
      const previousTime = nextState.currentTime ?? nextState.currentDay * dayLengthUnits;
      const updatedTime = previousTime + deltaTimeUnits;
      const previousDay = nextState.currentDay;
      const nextDay = Math.floor(updatedTime / dayLengthUnits);
      const daysAdvanced = Math.max(0, nextDay - previousDay);

      nextState.currentTime = updatedTime;
      nextState.currentDay = Math.max(nextDay, 0);

      // Update day/night cycle state
      const dayNightCycle = config.globalRules.dayNightCycle;
      if (dayNightCycle) {
        const totalCycleUnits = dayNightCycle.dayTimeUnits + dayNightCycle.nightTimeUnits;
        const timeInCurrentCycle = updatedTime % totalCycleUnits;
        const wasDayPhase = nextState.isDayPhase;
        
        nextState.isDayPhase = timeInCurrentCycle < dayNightCycle.dayTimeUnits;
        nextState.cycleProgress = nextState.isDayPhase 
          ? timeInCurrentCycle / dayNightCycle.dayTimeUnits
          : (timeInCurrentCycle - dayNightCycle.dayTimeUnits) / dayNightCycle.nightTimeUnits;
    ...
    queueMicrotask(() => {
      ...
      // Skip game over check during initial hydration (day 0, time 0)
      if (currentState.currentDay === 0 && currentState.currentTime === 0) {
        return;
      }
    ...
    queueMicrotask(() => {
      ...
      // Skip game over check during initial hydration (day 0, time 0)
      if (latestState.currentDay === 0 && latestState.currentTime === 0) {
